...
function getDraft(playerId){return drafts[draftKey(playerId)]||{scores:{},observation:""}}
...
function page(name,i){
 const pid=PLAYER_IDS[name], d=getDraft(pid);
 ...
 return `<section id="sp${i}" class="p"><h2>${name}</h2><p class="lead"><b>Mijn beoordeling</b> — ${reviewer()?.name||"kies eerst een beoordelaar op de homepage"}.</p>
 ...
 <div class="sync"><button class="b" id="save${i}" onclick="saveAssessment(${i})">Mijn beoordeling opslaan</button><span id="ss${i}">Lokale draft wordt automatisch bewaard</span></div>
...
function renderMyFit(i,scores){
 ...
 if(!isCompleteScores(scores)){e.innerHTML=`Nog ${METRICS.length-METRICS.filter(m=>scores?.[m.id]).length} score(s) invullen.`;return}
