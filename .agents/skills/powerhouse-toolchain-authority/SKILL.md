...
Netlify is authoritative for web-runtime deployment identity; GitHub remains source/delivery authority.

Rules:
1. Netlify `401 Unauthorized` is transport/auth failure, not product-code evidence.
...
4. If a GitHub readback is cancelled after provider identity is proven, preserve provider proof and rerun only missing functional/readback gates on the canonical current lineage.
5. If production advances, retain earlier deploy identity as historical proof and read current production separately.
