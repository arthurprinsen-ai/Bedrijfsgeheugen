import {runConnectorTest,evaluateActivationEvidence} from './connector-engine.mjs';
...
export function createConnectorRuntime({providers={}}={}){
  return Object.freeze({
    async runTest({connector,input}){
      const sourceProvider=providers.sources?.[connector?.source?.type];
      const extractorProvider=providers.extractor;
      const targetProvider=providers.targets?.[connector?.target?.type];
      const lookupProvider=providers.lookups;
...
export function createEnvironmentConnectorProviders({fetchFn=globalThis.fetch,env=process.env}={}){
  const sources={
    upload:{read:async input=>{if(!input?.content&&!input?.extractedFields)throw Object.assign(new Error('Safe-test sample is required'),{code:'SAFE_TEST_SAMPLE_REQUIRED'});return {content:input.content||'',hash:input.hash||`sample-${Date.now()}`,sample:input};}}
  };
  const extractor={extract:async source=>{
    const sample=source?.sample||{};
    if(!sample.extractedFields)throw Object.assign(new Error('Document extraction provider is not configured; provide explicit safe-test extractedFields or configure a server extractor.'),{code:'DOCUMENT_EXTRACTION_PROVIDER_NOT_CONFIGURED'});
    return {type:sample.documentType||'custom',confidence:Number(sample.classificationConfidence??1),fields:Object.fromEntries(Object.entries(sample.extractedFields).map(([key,value])=>[key,typeof value==='object'&&value&&'value'in value?value:{value,confidence:1}]))};
  }};
  const targets={
    datahub:{safeTest:async payload=>({ok:true,reference:'datahub-safe-test',payload})}
  };
