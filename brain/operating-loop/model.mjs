const TYPE_KIND={Evidence:'evidence',Signal:'signal',Opportunity:'opportunity',Decision:'decision',Action:'action',Outcome:'outcome',Learning:'learning',Entity:'entity',Relation:'relation',CurrentState:'current_state'};
...
export function normalizeBrainRecord(input){
  ...
  const subjectId=clean(input.subjectId||`${kind}:${id}`);const evidenceIds=unique(Array.isArray(input.evidenceIds)?input.evidenceIds.map(clean):[]);
  const references=unique([input.decisionId,input.actionId,input.outcomeId,...evidenceIds].map(clean));const nodes=unique([subjectId,...references]);
  const edges=references.map(target=>({from:id,to:target,relation:'supports_or_derives_from'}));
  return Object.freeze({schemaVersion:'brain-record.v1',tenantId,type,kind,id,subjectId,owner:clean(input.owner||'UNASSIGNED'),status:clean(input.status||'OBSERVED'),observedAt:input.observedAt||new Date().toISOString(),executed:input.executed===true,verified:input.verified===true,result:input.result??null,evidenceIds,references,payload:input.payload||{},provenance:{source:clean(input.source||input.provenance?.source||'brain'),sourceId:clean(input.sourceId||input.provenance?.sourceId||id)},graph:{nodes,edges}});
}
...
export function deriveLoopState(records){
  ...
  return Object.freeze({stages:{detect:evidence.length>0,verify:evidence.some(r=>Boolean(r.provenance?.source)),match:normalized.some(r=>Boolean(r.subjectId)),impact:decisions.length>0||outcomes.length>0,prioritise:decisions.length>0,recommend:advice.length>0,act:actions.length>0,measure:outcomes.length>0,learn:learnings.length>0},graph:graphFrom(normalized),advice,memory:{livingMemory:normalized},counts:{evidence:evidence.length,decisions:decisions.length,actions:actions.length,outcomes:outcomes.length,learnings:learnings.length}});
}
