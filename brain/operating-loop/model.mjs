...
export function deriveLoopState(records){
  const normalized=records.map(r=>r?.schemaVersion==='brain-record.v1'?r:normalizeBrainRecord(r));const evidence=normalized.filter(r=>r.kind==='evidence');const graphRecords=normalized.filter(r=>(r.kind==='entity'||r.kind==='relation')&&r.payload?.loopStage!=='graph_feedback');const intelligence=normalized.filter(r=>r.kind==='signal'||r.kind==='opportunity');const impacts=normalized.filter(r=>r.kind==='impact');const decisions=normalized.filter(r=>r.kind==='decision');const actions=normalized.filter(r=>r.kind==='action');const executions=normalized.filter(r=>r.kind==='execution');const verifications=normalized.filter(r=>r.kind==='verification');const outcomes=normalized.filter(r=>r.kind==='outcome');const values=normalized.filter(r=>r.kind==='value');const learnings=normalized.filter(r=>r.kind==='learning');const memories=normalized.filter(r=>r.kind==='memory');const governance=normalized.filter(r=>r.kind==='governance');const graphFeedback=normalized.filter(r=>r.kind==='relation'&&r.payload?.loopStage==='graph_feedback');
