const TYPE_KIND={Evidence:'evidence',Signal:'signal',Opportunity:'opportunity',Impact:'impact',Decision:'decision',Approval:'approval',Action:'action',Execution:'execution',Verification:'verification',Outcome:'outcome',Value:'value',Learning:'learning',Memory:'memory',Entity:'entity',Relation:'relation',CurrentState:'current_state',Governance:'governance'};
...
export function normalizeBrainRecord(input){
  ...
  const references=unique([input.decisionId,input.approvalId,input.actionId,input.executionId,input.verificationId,input.outcomeId,input.valueId,input.learningId,input.memoryId,...predecessorIds,...evidenceIds].map(clean));const nodes=unique([subjectId,...references]);
  ...
  const actor=clean(input.actor||input.performedBy||input.owner||'UNASSIGNED');
  const actorType=clean(input.actorType||input.performedByType||(String(actor).startsWith('agent:')?'agent':String(actor).startsWith('user:')?'human':'unknown'));
  const approval={state:clean(input.approvalState||input.approval?.state||'NOT_REQUIRED'),by:clean(input.approvedBy||input.approval?.by||'')||null,at:input.approvedAt||input.approval?.at||null,rationale:input.approvalRationale||input.approval?.rationale||null};
  const economics={cost:numberOrZero(input.costAmount??input.economics?.cost),expectedValue:numberOrZero(input.expectedValue??input.economics?.expectedValue),realizedValue:numberOrZero(input.realizedValue??input.realisedValue??input.economics?.realizedValue),currency:clean(input.currency||input.economics?.currency||'EUR')};
  return Object.freeze({schemaVersion:'brain-record.v1',tenantId,type,kind,id,subjectId,correlationId,predecessorIds,decisionId:clean(input.decisionId||'')||null,approvalId:clean(input.approvalId||'')||null,actionId:clean(input.actionId||'')||null,owner:clean(input.owner||'UNASSIGNED'),actor,actorType,status:clean(input.status||'OBSERVED'),observedAt:input.observedAt||new Date().toISOString(),executed:input.executed===true,verified:input.verified===true,result:input.result??null,evidenceIds,references,approval,economics,payload:input.payload||{},provenance:{source:clean(input.source||input.provenance?.source||'brain'),sourceId:clean(input.sourceId||input.provenance?.sourceId||id)},graph:{nodes,edges}});
}
...
export function deriveLoopState(records){
  const normalized=records.map(r=>r?.schemaVersion==='brain-record.v1'?r:normalizeBrainRecord(r));const evidence=normalized.filter(r=>r.kind==='evidence');const graphRecords=normalized.filter(r=>(r.kind==='entity'||r.kind==='relation')&&r.payload?.loopStage!=='graph_feedback');const intelligence=normalized.filter(r=>r.kind==='signal'||r.kind==='opportunity');const impacts=normalized.filter(r=>r.kind==='impact');const decisions=normalized.filter(r=>r.kind==='decision');const approvals=normalized.filter(r=>r.kind==='approval');const actions=normalized.filter(r=>r.kind==='action');const executions=normalized.filter(r=>r.kind==='execution');const verifications=normalized.filter(r=>r.kind==='verification');const outcomes=normalized.filter(r=>r.kind==='outcome');const values=normalized.filter(r=>r.kind==='value');const learnings=normalized.filter(r=>r.kind==='learning');const memories=normalized.filter(r=>r.kind==='memory');const governance=normalized.filter(r=>r.kind==='governance');const graphFeedback=normalized.filter(r=>r.kind==='relation'&&r.payload?.loopStage==='graph_feedback');
  ...
  return Object.freeze({stages:{...canonicalStages,...legacyStages},graph:graphFrom(normalized),advice,memory:{livingMemory:normalized},counts:{evidence:evidence.length,graph:graphRecords.length,intelligence:intelligence.length,impact:impacts.length,decisions:decisions.length,approvals:approvals.length,actions:actions.length,executions:executions.length,verifications:verifications.length,outcomes:outcomes.length,value:values.length,learnings:learnings.length,memory:memories.length,governance:governance.length,graphFeedback:graphFeedback.length}});
}
