...
export function createSupabaseCurrentStateInput({tenantId,table,rowId,updatedAt,operationStatus,operation,errorCode,revision}={}){
  ...
  return {
    tenantId:tenant,source:'supabase',id:`supabase-current-state:${tableName}:${row}`,component:`supabase:${tableName}`,
    raw:{table:tableName,row_id:row,updated_at:observedAt,operation:operation===undefined||operation===null?null:String(operation),operation_status:status,error_code:errorCode===undefined||errorCode===null?null:String(errorCode)},
    observedAt,health:mapped.health,error:mapped.error,owner:'supabase',cost:null,
    revision:revision===undefined||revision===null||String(revision).trim()===''?observedAt:String(revision).trim(),
    capacity:mapped.capacity,executionStatus:mapped.executionStatus,verified:true
  };
}
