...
export function createDataForSeoCurrentStateInput({tenantId,taskId,keyword,locationCode,taskState,observedAt,resultCount,errorMessage}={}){
  ...
  return {
    tenantId:tenant,source:'dataforseo',id:`dataforseo-current-state:${task}:${location}:${query}`,component:`dataforseo:${location}:${query}`,
    raw:{task_id:task,keyword:query,location_code:location,observed_at:observed,task_state:state,result_count:Number.isFinite(Number(resultCount))?Number(resultCount):null,error_message:errorMessage===undefined||errorMessage===null?null:String(errorMessage)},
    observedAt:observed,health:mapped.health,error:mapped.error,owner:'dataforseo',cost:null,revision:task,capacity:mapped.capacity,executionStatus:mapped.executionStatus,verified:true
  };
}
