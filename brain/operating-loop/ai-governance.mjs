...
function projectSystem(record,{now}={}){
  ...
  return Object.freeze({id:record.id,subjectId:record.subjectId,systemName:text(payload.systemName),provider:text(payload.provider),model:text(payload.model),purpose:text(payload.purpose),role:text(payload.role),riskLevel,classificationSource:text(payload.classificationSource),dataCategories:list(payload.dataCategories),humanOversight:Object.freeze({required:humanOversight.required===true,control:text(humanOversight.control)}),transparencyControl:text(payload.transparencyControl),loggingControl:text(payload.loggingControl),reviewDueAt,owner:text(record.owner)||'UNASSIGNED',evidenceIds,provenance:record.provenance||null,status:record.status||null,observedAt:record.observedAt||null,missing:uniqueMissing,readiness,productionAllowed:readiness==='EVIDENCED'});
}

export function projectAiGovernance(records=[],options={}){
  const systems=(Array.isArray(records)?records:[]).filter(record=>record&&(record.kind==='governance'||record.type==='Governance')).map(record=>projectSystem(record,options)).sort((a,b)=>String(a.systemName||a.subjectId).localeCompare(String(b.systemName||b.subjectId)));
