...
export function createOperatingLoopStore(adapter,{now=()=>new Date().toISOString()}={}){
  ...
  return Object.freeze({
    async append(input){
      ...
      const key=keyFor(record.tenantId,record.id);
      const existing=await adapter.get(key);
      if(existing){
        if(existing.idempotencyKey!==input.idempotencyKey){
          const error=new Error('Brain record id conflict');
          error.code='BRAIN_RECORD_CONFLICT';
          throw error;
        }
        return {duplicate:true,record:existing.record};
      }
      const envelope={idempotencyKey:String(input.idempotencyKey),storedAt:now(),record};
      await adapter.put(key,envelope);
      return {duplicate:false,record};
    },

    async getProjection(tenantId){
      const entries=await adapter.list(prefixFor(tenantId));
      const records=entries.map(entry=>entry.value?.record).filter(Boolean).sort((a,b)=>String(a.observedAt).localeCompare(String(b.observedAt)));
