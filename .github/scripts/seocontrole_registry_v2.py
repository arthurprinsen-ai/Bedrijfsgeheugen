...
Authority:
- site/seo-order-map.json + site/seo-order-expansion.json own primary keyword intent;
- bg-keyword-cluster projects the commercial cluster a page belongs to;
- supporting pages may project their owner's keyword only when intent-role and
  intent-owner explicitly prove that relationship;
...
def _body_attr(source: str, name: str) -> str:
    body = re.search(r'<body\b([^>]*)>', source, re.I)
    if not body:
        return ''
    m = re.search(r'\b%s=["\']([^"\']*)["\']' % re.escape(name), body.group(1), re.I)
...
def lees_paginas() -> dict[str, dict]:
    ...
    for path in candidate_files():
        ...
        pages[url] = {
            ...
            'intent_role': _meta(source, 'bg-intent-role') or _body_attr(source, 'data-bg-intent-role'),
            'intent_owner': _meta(source, 'bg-intent-owner') or _body_attr(source, 'data-bg-intent-owner'),
...
def keyword_owner_finding(route: str, page: dict, primary_owner: dict[str, str]):
    """Return blocker text only for a real ownership conflict.

    A supporting article is intentionally allowed to project its commercial
    owner's cluster keyword. It is safe only when the page explicitly says it
    is supporting and its intent-owner equals the registry's primary owner.
    """
    projected = norm(page.get('keyword', ''))
    owner = primary_owner.get(projected)
    if not owner or owner == route:
        return None
    role = norm(page.get('intent_role', ''))
    intent_owner = route_of_absolute(page.get('intent_owner', ''))
    if role == 'supporting' and intent_owner == owner:
        return None
    if role == 'supporting':
        return 'keyword-cluster "%s" is owned door %s maar supporting intent-owner is %s' % (
            page.get('keyword', ''), owner, intent_owner or 'ontbreekt')
