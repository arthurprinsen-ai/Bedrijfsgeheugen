...
def main():
    ...
    try:
        with sync_playwright() as pw:
            ...
            for naam in paginas:
                ...
                # Tijdens de V17/V18-migratie bestaan er twee expliciete canonical
                # representaties. De gate accepteert alleen die bekende contracten,
                # niet willekeurige header/footer-elementen.
                ...
                if koppen != 1:
                    fouten.append(
                        'canonical header ontbreekt of staat er dubbel (%d gevonden)' % koppen
                    )
                if voeten != 1:
                    fouten.append(
                        'canonical voettekst ontbreekt of staat er dubbel (%d gevonden)' % voeten
                    )
                if h1s != 1:
                ...
                vitals = pagina.evaluate(
                    '()=>({cls: window.__cls||0, lcp: Math.round(window.__lcp||0), '
                    'bronnen: Object.entries(window.__bronnen||{}).sort((a,b)=>b[1]-a[1])'
                    '.slice(0,3).map(x=>x[0]+" ("+x[1].toFixed(3)+")")})'
                )
                if vitals['cls'] > 0.1:
                    fouten.append(
                        'layout verspringt tijdens het laden (CLS %.3f, grens 0,1) — schuldig: %s'
                        % (
                            vitals['cls'],
                ...
                if vitals['lcp'] > 2500:
                    fouten.append(
                        'grootste element verschijnt pas na %d ms (grens 2500)'
                        % vitals['lcp']
                    )
                ...
                zonder_alt = pagina.eval_on_selector_all(
                    'img:not([alt])',
                    'e=>e.map(x=>x.getAttribute("src")||"?").slice(0,3)'
                )
                if zonder_alt:
    ...
    if problemen:
        ...
        regels.append(
            '**%d pagina%s met een probleem.**'
            % (aantal, '' if aantal == 1 else "'s")
        )
        regels.append('')
        ...
        regels.append(
            'Geen problemen gevonden. Elke publieke pagina heeft de canonical shell, '
            'precies één h1, geldige basis-metadata, geen JavaScript-fouten, geen dode '
            'interne links en past op een telefoonscherm.'
        )
