-- Canonical follow-up for production-proven full-cycle proof status normalization.
-- Root causes observed in production on 2026-09-15:
-- 1. source freshness view exposes lowercase ok/fresh while the proof compared uppercase;
-- 2. canonical GA4 batches may use status IMPORTED and must remain valid observed evidence.
...
values (
  'learning:full-cycle-status-normalization-v1',
  ...
  now(),
  jsonb_build_object(
    'incident','enum/casing mismatch caused false-red full-cycle proof',
    'root_cause','proof compared uppercase source/freshness values and excluded canonical IMPORTED GA4 batches',
    'prevention','normalize enum comparisons with lower() and explicitly accept canonical imported GA4 batches',
    'contract','powerhouse-full-cycle-production-proof-v1'
  ),
