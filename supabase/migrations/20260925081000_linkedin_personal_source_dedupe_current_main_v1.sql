...
-- Reconcile production hotfix into canonical Git without breaking already-patched production.
DO $do$
DECLARE
  v_def text;
  v_old text;
...
$new$;
  ...
  -- Production may already contain the hotfix from an earlier direct recovery.
  if position(v_new in v_def) > 0 then
    return;
  end if;

  if position(v_old in v_def) = 0 then
    raise exception 'linkedin personal fallback source-selection contract drifted; manual reconciliation required';
  end if;
