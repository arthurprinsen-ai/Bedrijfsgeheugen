-- Corrective migration for powerhouse-autonomous-improvement-runtime-v1.
-- Root cause: brain_records_kind_check does not allow the custom kind
-- `autonomous_improvement_cycle`. Reuse the canonical `current_state` kind and
-- retain the autonomous-cycle identity in record_id, subject/owner, payload and provenance.
...
begin
  ...
  where tenant_id='canonical'
    ...
    and payload->>'fingerprint'='powerhouse-autonomous-improvement-runtime-v1'
  ...
  v_packet := jsonb_build_object(
    ...
    'cycle_kind','autonomous_improvement_cycle',
    ...
    'writeback',jsonb_build_object('authority','supabase:brain_records','record_kind','current_state','cycle_kind','autonomous_improvement_cycle','idempotent',true,'new_store',false),
    'revalidate',jsonb_build_object('required',true,'next_cycle_hourly',true)
  );

  select * into v_record from public.brain_append_record(
    'canonical',v_record_id,'improvement','current_state','bedrijfsgeheugen-powerhouse',v_idempotency,
    ...
    jsonb_build_object('candidate_count',v_candidate_count,'writeback_verified',true,'cycle_kind','autonomous_improvement_cycle'),array[]::text[],
    jsonb_build_object('source','supabase_pg_cron','authority','canonical','freshness','live','cycle_kind','autonomous_improvement_cycle'),v_packet,v_idempotency,'powerhouse-autonomous-improvement-runtime-v1'
  );
  return v_record;
end;
$function$;
