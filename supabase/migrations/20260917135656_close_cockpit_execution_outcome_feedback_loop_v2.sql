...
declare
  ...
  v_revenue numeric := coalesce(p_revenue_eur,0);
  v_has_realized_revenue boolean := coalesce(p_revenue_eur,0) > 0;
...
begin
  ...
  if v_has_realized_revenue and v_type not in (
    'won','closed_won','order','won_order','order_won','realized_revenue','revenue_observed'
  ) then
    raise exception 'revenue not allowed for proposal or non-realized outcome';
  end if;
  if v_has_realized_revenue and coalesce(p_evidence,'{}'::jsonb) = '{}'::jsonb then
    raise exception 'REVENUE_SOURCE_EVIDENCE_REQUIRED';
  end if;
  if v_type in ('no_response','no_reply')
     and coalesce((p_evidence->>'observation_window_closed')::boolean,false) is not true
     and not v_human_ui then
    raise exception 'NO_RESPONSE_REQUIRES_CLOSED_OBSERVATION_WINDOW_OR_HUMAN_CONFIRMATION';
  end if;
  ...
  insert into public.powerhouse_sales_outcomes(
    ...
    v_revenue,coalesce(p_evidence,'{}'::jsonb)
  )
  ...
  set outcome_id=v_outcome.outcome_id,
      status=case
        when v_type in ('waiting','no_response','no_reply','defer','deferred','not_now') then 'waiting'
        else 'done'
      end,
      executed_at=case
        when v_type in ('defer','deferred','not_now') then executed_at
        else coalesce(executed_at,now())
      end,
      updated_at=now()
  ...
  if v_action.event_id is not null then
    ...
    where event_id=v_action.event_id;
  end if;

  if v_human_ui then
    v_feedback_type := case
      when v_type in ('not_relevant','defer','deferred','not_now','no_response','no_reply') then 'skip'
      else 'approve'
    end;
    perform public.powerhouse_record_human_feedback_v1(
      'cockpit-outcome:' || p_dedupe_key,
      v_feedback_type,
      v_action.action_id,
      v_action.opportunity_key,
      v_action.subject_key,
      'Human cockpit outcome: ' || v_type,
      v_action.message_draft,
      null,
      case when v_type in ('defer','deferred','not_now') then 'defer' else null end,
      coalesce(p_evidence,'{}'::jsonb) || jsonb_build_object('outcome_id',v_outcome.outcome_id,'outcome_type',v_type),
      coalesce(v_outcome.occurred_at,now())
    );
  end if;
