-- Powerhouse production/source parity + canonical Brain revenue projection.
-- Existing-state-first: this source-controls the function already proven in production.
-- No new table, scheduler, queue, CRM, analytics store or learning store is introduced.
...
-- Source-parity learning: production behavior must never exist only as an unsourced hotfix.
insert into public.powerhouse_runtime_events(
  ...
  'learning:powerhouse-production-source-parity-v1',
  'production_learning_recorded',
  'powerhouse-source-parity-instagram-v1',
  'production-source-drift',
  'system',now(),
  jsonb_build_object(
    'incident','canonical Brain revenue projection and deployed content Edge Functions existed in production without matching current-main source files',
    'root_cause','historical production hotfixes and superseded PR branches left production ahead of repository source truth',
    'prevention','PRODUCTION_SOURCE_PARITY_REQUIRED: every production database function, trigger and Edge Function change must be source-controlled on protected main and verified against deployed readback',
    'revenue_lineage',jsonb_build_array('powerhouse_forecasts','powerhouse_sales_actions','powerhouse_sales_outcomes','powerhouse_forecast_calibration'),
    'instagram_rule','instagram_company may execute only with verified Mira media evidence and a public HTTPS provider-usable asset URL'
  ),
  jsonb_build_object('contract','powerhouse-full-cycle-proof-v1','principle','EXISTING-STATE-FIRST','no_parallel_system',true),
