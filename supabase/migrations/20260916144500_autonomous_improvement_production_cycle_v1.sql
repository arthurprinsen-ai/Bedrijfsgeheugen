-- Canonical production adapter for powerhouse-autonomous-improvement-runtime-v1.
-- Reuses existing Supabase/Brain authority. No new store, queue, brain or scheduler authority.
...
begin
  ...
  where tenant_id='canonical' and record_kind='autonomous_improvement_cycle' and record_id<>v_record_id
  ...
  v_packet := jsonb_build_object(
    ...
    'writeback',jsonb_build_object('authority','supabase:brain_records','idempotent',true,'new_store',false),
    'revalidate',jsonb_build_object('required',true,'next_cycle_hourly',true)
  );

  select * into v_record from public.brain_append_record(
    'canonical',v_record_id,'improvement','autonomous_improvement_cycle','bedrijfsgeheugen-powerhouse',v_idempotency,
...
begin
  ...
  perform cron.schedule('powerhouse-autonomous-improvement-cycle-v1','42 * * * *','select public.powerhouse_autonomous_improvement_cycle_v1(now());');
end;
$schedule$;
