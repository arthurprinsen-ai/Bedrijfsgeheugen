-- Keep canonical scan ingestion compatible with the database DELETE safety guard.
-- The benchmark projection is a rebuildable derived projection; scan_inzendingen
-- remains the authority. Explicit WHERE true preserves the full refresh semantics
-- while satisfying the fail-closed DELETE contract.
...
begin
  ...
  where soort='frisse_blik' and score is not null and branche is not null and tenant_identity_status='verified' and organisatie_id is not null
