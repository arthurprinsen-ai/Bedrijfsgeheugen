...
declare
  ...
  v_proof jsonb;
begin
  -- Refresh canonical health state before evaluating source freshness.
  perform public.bg_gezondheid_meten();

  -- Reconcile only evidence-backed terminal publication states.
  v_reconciliation := public.powerhouse_reconcile_terminal_publication_state(p_run_date);

  -- Reuse the canonical daily execution guard after reconciliation.
  ...
  select
    ...
    coalesce(jsonb_agg(
      jsonb_build_object(
        ...
        'health', canonical_health_status,
  ...
  select max(uitgevoerd_op) filter (where status = 'ok')
  ...
  v_buffer_healthy :=
    v_buffer_last_sync is not null
  ...
  select max(uitgevoerd_op) filter (where status in ('ok', 'partial'))
    into v_ga4_last_sync
    from public.bg_ga4_sync;

  select max(created_at), coalesce(sum(rows_total) filter (where created_at >= v_now - interval '48 hours'), 0)::integer
    into v_ga4_last_batch, v_ga4_batch_rows
    from public.bg_ga4_csv_batches
   where status in ('ok', 'complete', 'completed', 'partial') or status is null;

  v_ga4_healthy :=
    v_ga4_last_sync is not null
  ...
  v_gmail_healthy :=
    v_gmail_last_readback is not null
  ...
  select count(*) filter (where measured_at >= v_now - interval '90 days'),
         max(measured_at)
    into v_calibrations_90d, v_last_calibration
    from public.powerhouse_forecast_calibration;

  v_healthy :=
    v_required_sources_healthy
  ...
  v_proof := jsonb_build_object(
    ...
    'ga4', jsonb_build_object('provider_layer', 'composio', 'last_sync', v_ga4_last_sync, 'last_batch', v_ga4_last_batch, 'batch_rows_48h', v_ga4_batch_rows),
  ...
  insert into public.powerhouse_runtime_events (
    ...
    'full-cycle-proof:' || p_run_date::text,
    'full_cycle_production_proof',
    'powerhouse_full_cycle_production_proof',
    p_run_date::text,
    'system',
    v_now,
    v_proof,
    jsonb_build_object('contract', 'powerhouse-full-cycle-production-proof-v1'),
    case when v_healthy then 'observed' else 'error' end,
    case when v_healthy then 'OBSERVED' else 'DEGRADED' end,
    1,
    v_now,
    v_now
  )
  ...
  values (
    v_now,
    'powerhouse-full-cycle-production-proof',
    'closed-loop-production-proof',
    case when v_healthy then 'OK' else 'FOUT' end,
    case when v_healthy then 'Volledige Powerhouse productiecyclus bewezen.' else 'Volledige Powerhouse productiecyclus nog niet volledig bewezen.' end,
    v_proof
  );
...
begin
  ...
  from cron.job
  where jobname = 'powerhouse-full-cycle-proof-hourly-v1'
  limit 1;
  ...
  perform cron.schedule(
    ...
    '57 * * * *',
    $cron$select public.powerhouse_full_cycle_production_proof((now() at time zone 'Europe/Amsterdam')::date);$cron$
  );
end;
$$;