...
declare v_event_id text:=nullif(trim(p_event->>'event_id'),''); v_event_type text:=nullif(trim(p_event->>'event_type'),''); v_canonical text:=nullif(trim(p_event->>'canonical'),''); v_intent_owner text:=coalesce(nullif(trim(p_event->>'intent_owner'),''),''); v_occurred timestamptz:=coalesce(nullif(p_event->>'occurred_at','')::timestamptz,now()); v_inserted integer:=0; v_fingerprint text;
begin
  ...
  values(v_event_id,v_event_type,v_canonical,nullif(p_event->>'intent',''),nullif(p_event->>'intent_owner',''),nullif(p_event->>'attribution_root_key',''),nullif(p_event->>'source',''),nullif(p_event->>'medium',''),nullif(p_event->>'campaign',''),v_occurred,nullif(p_event->>'page_role',''),nullif(p_event->>'funnel_stage',''),coalesce((p_event->>'value')::numeric,0),coalesce(p_event->'payload','{}'::jsonb)) on conflict(event_id) do nothing;
  get diagnostics v_inserted=row_count;
  insert into public.growth_brain_queue(queue_id,kind,source_id,fingerprint,state) values('event:'||v_event_id,'event',v_event_id,v_fingerprint,'QUEUED') on conflict(kind,source_id) do nothing;
  if v_inserted=1 and v_canonical is not null then
    insert into public.growth_page_daily(day,canonical,intent_owner,page_views,cta_clicks) values(v_occurred::date,v_canonical,v_intent_owner,case when v_event_type='page_view' then 1 else 0 end,case when v_event_type in ('cta_click','selfscan_start','frisse_blik_click') then 1 else 0 end)
    on conflict(day,canonical,intent_owner) do update set page_views=public.growth_page_daily.page_views+excluded.page_views,cta_clicks=public.growth_page_daily.cta_clicks+excluded.cta_clicks,updated_at=now();
  end if;
...
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_id text:=nullif(trim(p_outcome->>'outcome_id'),''); v_stage text:=nullif(trim(p_outcome->>'stage'),''); v_root text:=nullif(trim(p_outcome->>'attribution_root_key'),''); v_canonical text:=nullif(trim(p_outcome->>'canonical'),''); v_intent_owner text:=coalesce(nullif(trim(p_outcome->>'intent_owner'),''),''); v_occurred timestamptz:=coalesce(nullif(p_outcome->>'occurred_at','')::timestamptz,now()); v_revenue numeric:=coalesce((p_outcome->>'revenue_eur')::numeric,0); v_inserted integer:=0; v_fingerprint text;
begin
  ...
  insert into public.growth_outcomes(outcome_id,stage,attribution_root_key,canonical,intent_owner,occurred_at,revenue_eur,source,payload) values(v_id,v_stage,v_root,v_canonical,nullif(p_outcome->>'intent_owner',''),v_occurred,v_revenue,nullif(p_outcome->>'source',''),coalesce(p_outcome->'payload','{}'::jsonb)) on conflict(outcome_id) do nothing;
  get diagnostics v_inserted=row_count;
  insert into public.growth_brain_queue(queue_id,kind,source_id,fingerprint,state) values('outcome:'||v_id,'outcome',v_id,v_fingerprint,'QUEUED') on conflict(kind,source_id) do nothing;
  if v_inserted=1 and v_canonical is not null then
    insert into public.growth_page_daily(day,canonical,intent_owner,leads,qualified_leads,appointments,proposals,won_orders,revenue_eur) values(v_occurred::date,v_canonical,v_intent_owner,case when v_stage='lead' then 1 else 0 end,case when v_stage='qualified_lead' then 1 else 0 end,case when v_stage='appointment' then 1 else 0 end,case when v_stage='proposal' then 1 else 0 end,case when v_stage='won_order' then 1 else 0 end,case when v_stage in ('won_order','revenue') then v_revenue else 0 end)
    on conflict(day,canonical,intent_owner) do update set leads=public.growth_page_daily.leads+excluded.leads,qualified_leads=public.growth_page_daily.qualified_leads+excluded.qualified_leads,appointments=public.growth_page_daily.appointments+excluded.appointments,proposals=public.growth_page_daily.proposals+excluded.proposals,won_orders=public.growth_page_daily.won_orders+excluded.won_orders,revenue_eur=public.growth_page_daily.revenue_eur+excluded.revenue_eur,updated_at=now();
  end if;
