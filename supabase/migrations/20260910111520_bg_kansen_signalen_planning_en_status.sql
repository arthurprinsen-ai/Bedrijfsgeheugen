select cron.schedule('bg-zoekwoordkansen-wekelijks', '0 6 * * 1', $c$select net.http_post(url := 'https://adhjwmvyoixzjtmiroln.supabase.co/functions/v1/bg-zoekwoordkansen', body := '{}'::jsonb, headers := '{"content-type":"application/json"}'::jsonb, timeout_milliseconds := 120000);$c$);
select cron.schedule('bg-externe-signalen-dagelijks', '30 5 * * *', $c$select net.http_post(url := 'https://adhjwmvyoixzjtmiroln.supabase.co/functions/v1/bg-externe-signalen', body := '{}'::jsonb, headers := '{"content-type":"application/json"}'::jsonb, timeout_milliseconds := 120000);$c$);
