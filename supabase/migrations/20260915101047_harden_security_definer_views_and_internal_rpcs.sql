...
begin
  foreach v in array array[
    'bg_connectie_activiteit','bg_connecties_stand','bg_connectiescore','bg_machine_status','bg_paginarendement','bg_post_prestatie','bg_trechter_dag','bg_vandaag','bg_verkeer_verdacht','content_operations_cockpit','powerhouse_first_mover_queue'
  ] loop
...
values(
  'public-security-definer-surface-regression-v1','OBSERVED',
  ...
  'powerhouse-security-definer-surface-v1',1,1,now(),now(),
  jsonb_build_object('detected_on','2026-09-15','contract','powerhouse-security-definer-surface-v1','views_hardened',11,'internal_rpcs_hardened',5,'intentional_public_exception','bg_klik_vastleggen(text,text)','exception_reason','public /g/... click attribution; destination allow-listed to bedrijfsgeheugen.nl')
)
on conflict (fingerprint) do update
set root_cause=excluded.root_cause,proven_fix=excluded.proven_fix,prevention_rule=excluded.prevention_rule,regression_ref=excluded.regression_ref,occurrence_count=public.brain_failure_registry.occurrence_count + 1,last_seen_at=now(),evidence=coalesce(public.brain_failure_registry.evidence,'{}'::jsonb)||excluded.evidence;
