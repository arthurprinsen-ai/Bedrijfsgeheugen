-- Reconcile revenue-intelligence health with executable commercial truth.
-- Research/enrichment work stays visible as research and does not masquerade as an executable lineage failure.
-- Runtime incidents count only when the latest state for the same event/source/subject is still error.
