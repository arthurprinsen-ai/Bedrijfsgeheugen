...
alter view public.powerhouse_buying_committee_v1 set (security_invoker = true);
...
where o.status='open';
alter view public.powerhouse_portfolio_allocator_v1 set (security_invoker = true);
...
from offers
group by offer_key;
alter view public.powerhouse_offer_pricing_learning_v1 set (security_invoker = true);
...
group by k.hook_type,k.format,k.narrative_type,k.emotion,k.cta_type,k.persona,k.pain_trigger,
  k.fomo_trigger,k.comedy_device,k.proof_type,k.offer_type,k.awareness_stage,k.behavioral_lever;
alter view public.powerhouse_creative_commercial_learning_v1 set (security_invoker = true);
...
alter view public.powerhouse_champion_challenger_v1 set (security_invoker = true);
...
alter view public.powerhouse_commercial_maturity_v1 set (security_invoker = true);
...
begin
  ...
  set expected_revenue_value = greatest(coalesce(o.expected_revenue_value,0),coalesce(pa.forecast_modeled_value_eur,0)),
      score_components = coalesce(o.score_components,'{}'::jsonb) || jsonb_build_object('commercial_learning_v1',jsonb_build_object('modeled_value_eur',pa.allocatable_modeled_value_eur,'portfolio_score_eur',pa.portfolio_score_eur,'allocation_confidence',pa.allocation_confidence,'economic_buyers',pa.economic_buyers,'technical_evaluators',pa.technical_evaluators,'champions',pa.champions,'truth_boundary','modeled expected value is not realized revenue and does not overwrite declared/observed expected_value_eur','scored_at',v_now)),
      updated_at=v_now
  ...
  insert into public.powerhouse_sales_actions(dedupe_key,subject_key,person_key,company_key,action_type,channel,priority,reason,evidence,message_draft,status,due_at,topic_key,opportunity_key,expected_value_eur)
  select 'commercial-learning:'||p_run_date::text||':'||r.opportunity_key,r.opportunity_key,r.person_key,r.company_key,
    coalesce(o.score_components#>>'{next_best_action,action}','research_enrichment'),coalesce(o.score_components#>>'{next_best_action,channel}','internal'),
    ...
    jsonb_build_object('contract','powerhouse-commercial-learning-v1','portfolio_score_eur',r.portfolio_score_eur,'modeled_value_eur',r.allocatable_modeled_value_eur,'allocation_confidence',r.allocation_confidence,'buying_committee',jsonb_build_object('economic_buyers',r.economic_buyers,'technical_evaluators',r.technical_evaluators,'champions',r.champions),'truth_boundary','existing execution eligibility, destination, identity, consent, dedupe, contact-pressure and provider-readback gates remain mandatory'),
    '','suggested',v_now,r.topic_key,r.opportunity_key,r.allocatable_modeled_value_eur
  from ranked r join public.powerhouse_opportunities o on o.opportunity_key=r.opportunity_key where r.rn<=5
  ...
  with observed as (
    select e.experiment_id,count(distinct k.post_key) as observed_posts,count(o.outcome_id) as commercial_outcomes,coalesce(sum(o.revenue_eur),0) as revenue_eur
    from public.social_experiments e left join public.bg_post_kenmerken k on k.experiment_id=e.experiment_id left join public.powerhouse_sales_outcomes o on o.content_key=k.post_key
    where e.tenant_id='canonical' group by e.experiment_id
  )
  update public.social_experiments e
  set resultaat=coalesce(e.resultaat,'{}'::jsonb) || jsonb_build_object('commercial_learning_v1',jsonb_build_object('observed_posts',ob.observed_posts,'commercial_outcomes',ob.commercial_outcomes,'realized_revenue_eur',ob.revenue_eur,'evaluated_at',v_now,'truth_boundary','no promotion decision without minimum observed sample and commercial evidence')),
      advies=case when coalesce(ob.observed_posts,0)<greatest(1,coalesce(e.min_steekproef,1)) then 'CONTINUE_MEASURING' when ob.commercial_outcomes>0 or ob.revenue_eur>0 then 'COMMERCIAL_EVIDENCE_PRESENT_REVIEW_FOR_PROMOTION' else 'HOLD_NO_COMMERCIAL_EVIDENCE_YET' end,
      updated_at=v_now
  from observed ob where ob.experiment_id=e.experiment_id and e.status in ('ACTIVE','PLANNED');
  ...
  v_result := jsonb_build_object('contract','powerhouse-commercial-learning-v1','run_date',p_run_date,'executed_at',v_now,'healthy',coalesce((v_gap->>'healthy')::boolean,false),'opportunities_enriched',v_enriched,'portfolio_actions_touched',v_actions,'experiments_evaluated',v_exp_evaluated,'maturity',v_maturity,'capabilities',jsonb_build_array('revenue_attribution','opportunity_economics','outcome_funnel','forecast_calibration','experiment_to_revenue','champion_challenger','portfolio_allocation','buying_committee','delivery_sales_learning','pricing_offer_learning','creative_commercial_learning','freshness_degradation'),'truth_boundary','realized revenue only from observed outcomes; modeled economic value is explicitly labeled and never promoted to realized revenue');
  ...
  values('powerhouse-commercial-learning-v1','powerhouse','growth_revenue_os','Commercial decisions improve when outcome, modeled economics, forecast calibration, experiments, buying committee, pricing and creative evidence are evaluated in one canonical lineage.',v_result,jsonb_build_object('primary_effect','commercial_feedback_compounds_into_next_decision','no_parallel_system',true),case when coalesce((v_gap->>'healthy')::boolean,false) then 0.90 else 0.45 end,'active',greatest(1,v_enriched+v_actions+v_exp_evaluated),v_now+interval '30 days')
  on conflict(fingerprint) do update set evidence=excluded.evidence,effect=excluded.effect,confidence=excluded.confidence,sample_size=excluded.sample_size,expires_at=excluded.expires_at,updated_at=v_now;
  ...
  values('commercial-learning:'||to_char(v_now at time zone 'UTC','YYYYMMDDHH24'),'commercial_learning_cycle','powerhouse-commercial-learning-v1','powerhouse',v_now,v_result,jsonb_build_object('existing_state_first',true,'reuse_first',true,'closed_loop',true),case when coalesce((v_gap->>'healthy')::boolean,false) then 'actioned' else 'error' end,'verified',1,v_now)
...
exception when others then
  insert into public.powerhouse_runtime_events(dedupe_key,event_type,source,subject_key,occurred_at,evidence,context,state,data_quality,confidence,updated_at)
  values('commercial-learning-error:'||to_char(now() at time zone 'UTC','YYYYMMDDHH24MI'),'commercial_learning_cycle_failed','powerhouse-commercial-learning-v1','powerhouse',now(),jsonb_build_object('error',sqlerrm,'sqlstate',sqlstate),jsonb_build_object('fail_closed',true),'error','verified',1,now()) on conflict(dedupe_key) do nothing;
