alter table public.social_posts add column if not exists channel_id text;
alter table public.social_posts add column if not exists channel_name text;
alter table public.social_posts add column if not exists channel_kind text;

create index if not exists social_posts_tenant_channel_kind_published_idx
  on public.social_posts (tenant_id, channel_kind, published_at desc);
...
begin
  ...
  where tenant_id = new.tenant_id and calendar_date = v_date
  order by updated_at desc nulls last, started_at desc nulls last
  limit 1;
  if not found then return new; end if;

  v_channel_kind := lower(coalesce(new.channel_kind,''));
  v_recipe := case
    when v_channel_kind = 'linkedin_company' then coalesce(v_experiment.recipe->'company','{}'::jsonb)
    when v_channel_kind = 'linkedin_personal' then coalesce(v_experiment.recipe->'personal','{}'::jsonb)
    when v_channel_kind in ('instagram_company','facebook_company') then coalesce(v_experiment.recipe->'company','{}'::jsonb)
  ...
  insert into public.bg_post_kenmerken(
    ...
    new.post_id,new.hook_type,new.format,new.narrative_type,new.emotion,new.cta_type,new.topic,
    ...
    nullif(v_experiment.source_signals->>0,''),v_experiment.commercial_hypothesis,v_experiment.experiment_id,
    ...
    nullif(coalesce(v_recipe->>'mentalAvailabilityCue',v_recipe->>'mental_availability_cue'),''),now()
  )
  on conflict (post_key) do update set
    hook_type=excluded.hook_type,format=excluded.format,narrative_type=excluded.narrative_type,
    emotion=excluded.emotion,cta_type=excluded.cta_type,topic=excluded.topic,bron=excluded.bron,
    pain_trigger=excluded.pain_trigger,fomo_trigger=excluded.fomo_trigger,comedy_device=excluded.comedy_device,
    proof_type=excluded.proof_type,offer_type=excluded.offer_type,source_signal=excluded.source_signal,
    commercial_hypothesis=excluded.commercial_hypothesis,experiment_id=excluded.experiment_id,
    objective=excluded.objective,awareness_stage=excluded.awareness_stage,behavioral_lever=excluded.behavioral_lever,
    category_entry_point=excluded.category_entry_point,mental_availability_cue=excluded.mental_availability_cue,
    bijgewerkt_op=now();
  return new;
end;
$$;
...
before insert or update of published_at, platform, channel_kind, source_campaign_id, format, hook_type, narrative_type, emotion, cta_type, topic
on public.social_posts
for each row execute function public.sync_social_post_content_chain();
