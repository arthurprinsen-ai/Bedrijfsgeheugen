-- Powerhouse Revenue Command Center projection.
-- Canonical facts remain runtime events/actions/outcomes/learnings; this migration adds a bounded commercial read model.
...
create index if not exists powerhouse_opportunities_value_idx
  on public.powerhouse_opportunities(status, expected_revenue_eur desc, next_action_at nulls first);
...
create index if not exists powerhouse_opportunity_stage_events_idx
  on public.powerhouse_opportunity_stage_events(opportunity_id, occurred_at desc);
create index if not exists powerhouse_actions_opportunity_idx
  on public.powerhouse_sales_actions(opportunity_key, status, priority desc);
...
language sql stable security definer set search_path=public as $$
  with ranked as (
    select
      a.*,
      coalesce(a.opportunity_key, a.subject_key, a.person_key, a.company_key, a.action_id::text) as resolved_opportunity_key,
      row_number() over (
        partition by coalesce(a.opportunity_key, a.subject_key, a.person_key, a.company_key, a.action_id::text)
  ...
  select
    r.action_id,
    o.opportunity_id,
    r.resolved_opportunity_key,
    coalesce(o.subject_key,r.subject_key),
    coalesce(o.person_key,r.person_key),
    coalesce(o.company_key,r.company_key),
    coalesce(o.person_name,r.person_name),
    coalesce(o.company_name,r.company_name),
    coalesce(o.role,r.role),
    coalesce(o.stage,'opportunity'),
    coalesce(o.status,'open'),
    coalesce(o.expected_order_value_eur,r.expected_value_eur,0),
    coalesce(o.order_probability,0.05),
    coalesce(o.probability_confidence,0.2),
    coalesce(o.expected_revenue_eur,coalesce(r.expected_value_eur,0)*0.05),
    r.action_type,
    r.channel,
    r.priority,
    r.reason,
  ...
  from ranked r
  left join public.powerhouse_opportunities o on o.opportunity_key=r.resolved_opportunity_key
  where r.rn=1
  order by
    coalesce(o.expected_revenue_eur,coalesce(r.expected_value_eur,0)*0.05) desc,
    r.priority desc,
    r.created_at asc
