...
create index if not exists powerhouse_experiment_assignments_opportunity_idx
  on public.powerhouse_experiment_assignments(opportunity_key, assigned_at desc);
create index if not exists powerhouse_experiment_assignments_horizon_idx
  on public.powerhouse_experiment_assignments(measurement_horizon_end, assignment_arm);
...
comment on table public.powerhouse_experiment_assignments is
'Canonical pre-treatment assignment authority. Stable candidate hashing is not causal proof; assignment must be persisted before treatment.';
comment on table public.powerhouse_action_economics is
'Observed action cost and human effort only. Missing cost/effort stays NULL and is never synthesized.';
comment on table public.powerhouse_human_feedback_events is
'First-class human approval/edit/skip/cancel/override/alternative-action evidence linked to the canonical sales lineage.';
...
declare
  ...
  v_hash text;
begin
  if nullif(btrim(p_experiment_key),'') is null or nullif(btrim(p_subject_key),'') is null then
    raise exception 'experiment_key and subject_key are required';
  end if;
  if p_measurement_horizon_end is null or p_measurement_horizon_end <= now() then
    raise exception 'measurement_horizon_end must be in the future at assignment time';
  end if;
  if p_assignment_arm is not null and p_assignment_arm not in ('treatment','holdout') then
    raise exception 'assignment_arm must be treatment or holdout';
  end if;
  ...
  if found then
    if p_assignment_arm is not null and v_existing.assignment_arm <> p_assignment_arm then
      raise exception 'existing assignment arm is immutable';
    end if;
    return v_existing;
  end if;
  ...
  v_arm := coalesce(
    p_assignment_arm,
    case when mod(abs(hashtextextended(p_experiment_key || '|' || p_subject_key, 0)::numeric), 5) = 0
  ...
  insert into public.powerhouse_experiment_assignments (
    ...
    v_dedupe, p_experiment_key, p_subject_key, p_opportunity_key, v_arm,
    coalesce(p_eligibility_snapshot,'{}'::jsonb), now(), p_measurement_horizon_end, p_model_version, v_hash
...
create or replace function public.powerhouse_link_experiment_action_v1(
...
begin
  ...
  if v_assignment.opportunity_key is not null and v_action.opportunity_key is distinct from v_assignment.opportunity_key then
    raise exception 'action opportunity does not match assignment';
  end if;
  if v_assignment.treatment_action_id is not null and v_assignment.treatment_action_id <> p_action_id then
    raise exception 'treatment action linkage is immutable';
  end if;

  update public.powerhouse_experiment_assignments
  set treatment_action_id = p_action_id,
      state = case when state = 'assigned' then 'treated' else state end,
      updated_at = now()
...
begin
  ...
  insert into public.powerhouse_human_feedback_events (
    ...
    p_dedupe_key, p_action_id, p_opportunity_key, p_subject_key, p_feedback_type, p_reason,
    p_recommended_variant, p_actual_variant, p_alternative_action, coalesce(p_observed_at,now()), coalesce(p_evidence,'{}'::jsonb)
  )
  on conflict (dedupe_key) do nothing
  returning * into v_row;

  if v_row.feedback_id is null then
    select * into v_row from public.powerhouse_human_feedback_events where dedupe_key = p_dedupe_key;
  end if;
  return v_row;
end;
$$;
...
with assignment_facts as (
  select
    a.*,
    sa.created_at as treatment_created_at,
    ...
    exists (
      ...
      where o.opportunity_key is not distinct from a.opportunity_key
        and o.occurred_at >= a.assigned_at
        and o.occurred_at <= a.measurement_horizon_end
    ) as observed_outcome,
    coalesce((
      select sum(o.revenue_eur) from public.powerhouse_sales_outcomes o
      where o.opportunity_key is not distinct from a.opportunity_key
        and o.occurred_at >= a.assigned_at
        and o.occurred_at <= a.measurement_horizon_end
...
select
  g.*,
  case
    when not coalesce(g.assignment_before_treatment,false) then 'not_proven'
    when g.matured_treatment < 10 or g.matured_holdout < 10 then 'insufficient_evidence'
    else 'ready_for_estimation'
  end as causal_status,
  case
    when not coalesce(g.assignment_before_treatment,false) then 'assignment_after_treatment_detected'
    when g.matured_treatment < 10 or g.matured_holdout < 10 then 'minimum_10_matured_per_arm_not_met'
    else 'comparable_estimation_may_begin_but_is_not_itself_causal_proof'
...
select
  ...
  e.external_cost_eur,
  e.human_minutes,
  case when e.provider_cost_eur is null and e.external_cost_eur is null then null
       else coalesce(e.provider_cost_eur,0) + coalesce(e.external_cost_eur,0) end as observed_cost_eur,
  ...
  case when (coalesce(e.provider_cost_eur,0)+coalesce(e.external_cost_eur,0)) > 0
    then coalesce(sum(o.revenue_eur),0) / (coalesce(e.provider_cost_eur,0)+coalesce(e.external_cost_eur,0))
    else null end as realized_revenue_to_cost_ratio,
...
group by a.action_id,a.opportunity_key,a.action_type,a.channel,a.status,a.executed_at,
         e.economics_id,e.provider_cost_eur,e.external_cost_eur,e.human_minutes;
...
left join public.powerhouse_sales_outcomes o
  on o.action_id = f.action_id and o.occurred_at >= f.observed_at
...
begin
  ...
  set evidence = jsonb_set(coalesce(evidence,'{}'::jsonb), '{market_truth_learning}', coalesce(v_health,'{}'::jsonb), true),
      updated_at = now()
  ...
  select max(occurred_at) into v_error_time
  from public.powerhouse_runtime_events
  where event_type='source_health_evaluated' and subject_key='ga4-analytics' and state='error';
