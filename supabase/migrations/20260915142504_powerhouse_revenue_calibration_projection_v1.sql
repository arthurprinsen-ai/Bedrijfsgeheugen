-- Project canonical Brain commercial predictions and settlements into the
-- existing Powerhouse forecast -> action -> outcome -> calibration lineage.
-- No parallel store, queue, CRM or learning system.
...
begin
  ...
  if learning_type = 'revenue_prediction' then
    ...
    insert into public.powerhouse_forecasts(
      ...
      greatest(0,least(1,coalesce((prediction->'prediction'->>'confidence')::numeric,0))),30,
    ...
    on conflict (forecast_key) do update set
      probability=excluded.probability,confidence=excluded.confidence,revenue_potential=excluded.revenue_potential,
      evidence=excluded.evidence,last_scored_at=excluded.last_scored_at,updated_at=now()
    ...
    insert into public.powerhouse_sales_actions(
      ...
      'brain-revenue-action:'||new.tenant_id||':'||decision_key,entity_key,entity_key,
      coalesce(nullif(prediction->'recommended_action'->>'type',''),'observe_and_enrich'),'brain',
      greatest(0,least(100,coalesce((prediction->'prediction'->>'opportunity_score')::numeric,0))),
      ...
      '','brain://revenue-calibration-loop','prepared',predicted_at,'commercial-meeting',decision_key,
      greatest(0,coalesce((prediction->'prediction'->>'expected_commercial_value')::numeric,0))
    )
    ...
    select forecast_id into forecast_uuid from public.powerhouse_forecasts
    ...
    select action_id into action_uuid from public.powerhouse_sales_actions
      where dedupe_key='brain-revenue-action:'||new.tenant_id||':'||decision_key;

    insert into public.powerhouse_sales_outcomes(
      action_id,dedupe_key,outcome_type,subject_key,company_key,revenue_eur,evidence,occurred_at,opportunity_key,channel
    ) values (
    ...
    update public.powerhouse_sales_actions
      set status='done',executed_at=coalesce(executed_at,occurred_at),outcome_id=outcome_uuid,updated_at=now()
      where dedupe_key='brain-revenue-action:'||new.tenant_id||':'||decision_key;
    ...
    if not exists(
      ...
      where c.forecast_id=forecast_uuid and c.evidence->>'canonical_brain_record_id'=new.record_id
    ) then
