...
-- Pin search_path for all functions currently flagged by Supabase Security Advisor.
-- Keep public first because these functions intentionally reference canonical public-schema
-- Powerhouse objects by unqualified name; pg_catalog is explicit and deterministic.
...
values(
  'mutable-function-search-path-regression-v1',
  'OBSERVED',
  'Public-schema Powerhouse functions were created without an explicit search_path, so object resolution depended on the caller/session search_path. Supabase Security Advisor flagged 16 functions.',
  'Pin each affected function search_path to the canonical public schema plus pg_catalog without changing function logic or privileges.',
  'Every created or replaced public-schema function must declare a deterministic search_path in the same migration. Mutable search_path warnings are security regressions and must fail release review.',
  'powerhouse-function-search-path-v1',
  1,1,now(),now(),
  jsonb_build_object('detected_on','2026-09-15','contract','powerhouse-function-search-path-v1','functions_hardened',16,'search_path','public, pg_catalog')
)
