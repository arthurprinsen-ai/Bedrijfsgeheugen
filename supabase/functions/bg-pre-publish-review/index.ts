...
import { validateMiraFinalAssetEvidence } from './mira-final-asset-gate.mjs';
...
const CHANNEL_ALIASES:Record<string,string>={linkedin_personal:'linkedin_personal',[PERSONAL_CHANNEL]:'linkedin_personal',arthurprinsen:'linkedin_personal',linkedin_company:'linkedin_company',[COMPANY_CHANNEL]:'linkedin_company',bedrijfsgeheugen:'linkedin_company',instagram_company:'instagram_company',instagram:'instagram_company',[INSTAGRAM_CHANNEL]:'instagram_company','bedrijfsgeheugen.nl':'instagram_company',instagram_personal:'instagram_personal'};
...
function personalViolations(text:string,body:any,hash:string){
  const out:any[]=[]; const add=(code:string,message:string)=>out.push({code,message}); const req=(ok:boolean,code:string,message:string)=>{if(!ok)add(code,message)};
...
function otherIdentityViolations(channel:string,text:string,body:any){
  ...
  if(channel==='instagram_company'){
    const mt=clean(body.media_type).toLowerCase(),src=clean(body.media_source).toLowerCase();
...
Deno.serve(async req=>{
  ...
  const url=Deno.env.get('SUPABASE_URL'),key=Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'); if(!url||!key)return json({ok:false,can_generate:false,can_publish:false,error:'SERVER_CONFIG'},503);
  let body:any={};try{body=await req.json()}catch{return json({ok:false,can_generate:false,can_publish:false,error:'INVALID_JSON'},400)}
  ...
  const parent=(contracts||[]).find((x:any)=>x.record_id===PARENT_CONTRACT),personal=(contracts||[]).find((x:any)=>x.record_id===PERSONAL_CONTRACT);
  ...
  const raw=clean(body.channel||body.channel_id||body.buffer_channel_id).toLowerCase(); const channel=CHANNEL_ALIASES[raw]; if(!channel)return json({ok:false,can_generate:false,can_publish:false,error:'CHANNEL_REQUIRED_OR_UNKNOWN',contract_id:PARENT_CONTRACT},422);
  const {data,error}=await db.from('bg_schrijfregels').select('regel_id,onderwerp,regel,onderbouwing,bewijs_n,vertrouwen,bron,status,bijgewerkt_op').order('vertrouwen',{ascending:false}).limit(24); if(error)return json({ok:false,can_generate:false,can_publish:false,error:'RULE_CONTEXT_UNAVAILABLE'},503);
  const all=Array.isArray(data)?data:[],active=all.filter((r:any)=>clean(r.status).toLowerCase()==='actief'&&clean(r.regel)&&clean(r.bijgewerkt_op)); if(!active.length)return json({ok:false,can_generate:false,can_publish:false,error:'RULE_CONTEXT_EMPTY'},503);
  const times=active.map((r:any)=>Date.parse(r.bijgewerkt_op)).filter(Number.isFinite); if(times.length!==active.length)return json({ok:false,can_generate:false,can_publish:false,error:'RULE_CONTEXT_INVALID'},503); const newest=Math.max(...times); if(Date.now()-newest>MAX_AGE)return json({ok:false,can_generate:false,can_publish:false,error:'RULE_CONTEXT_STALE'},503);
  const rules=active.slice(0,12).map((r:any)=>({id:r.regel_id,topic:r.onderwerp,rule:r.regel,evidence:r.onderbouwing,evidence_count:r.bewijs_n,confidence:Number(r.vertrouwen||0),source:r.bron,updated_at:r.bijgewerkt_op}));
  ...
  let identityBlockers:any[]=channel==='linkedin_personal'?personalViolations(text,body,hash):otherIdentityViolations(channel,text,body);
  if(channel==='instagram_company'){
    const evidenceRecordId=clean(body.mira_evidence_record_id);
    let evidenceRecord:any=null;
    if(evidenceRecordId){
      const {data:miraEvidence,error:miraEvidenceError}=await db.from('brain_records').select('record_id,status,verified,payload,result,updated_at').eq('tenant_id','canonical').eq('record_id',evidenceRecordId).maybeSingle();
      if(miraEvidenceError)return json({ok:false,can_generate:false,can_publish:false,error:'MIRA_EVIDENCE_UNAVAILABLE',channel},503);
      evidenceRecord=miraEvidence;
    }
    identityBlockers=[...identityBlockers,...validateMiraFinalAssetEvidence({
      gatePassed:body.mira_gate_passed,
      finalAssetUrl:body.final_asset_url,
      calendarRowId:body.calendar_row_id,
      evidenceRecordId,
      evidenceRecord,
      nowMs:Date.now(),
    })];
  ...
  if(channel!=='linkedin_personal'){
    const {data:violations,error:checkError}=await db.rpc('bg_brein_regels_check',{p_connectie_id:clean(body.connectie_id)||null,p_tekst:text,p_haaktype:clean(body.hook_type)||null}); if(checkError)return json({ok:false,can_generate:false,can_publish:false,error:'RULE_CHECK_FAILED',channel},503);
    genericBlockers=(violations||[]).filter((v:any)=>v.violation);
  }
  const blockers=[...identityBlockers,...genericBlockers]; const pass=blockers.length===0;
