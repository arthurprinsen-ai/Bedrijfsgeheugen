...
Deno.serve(async(req:Request)=>{
  ...
  if(body?.action==='claim'){
    ...
    if(!uuidRe.test(tenantId)||!validKey(submissionKey))return json({error:'IDENTITY_NOT_ORGANISATION'},422);
    const {data:org,error:orgError}=await client.from('organisaties').select('id,slug,naam').eq('id',tenantId).maybeSingle();
    if(orgError||!org)return json({error:'ORGANISATION_NOT_FOUND'},404);
    ...
    const {data:claimed,error:claimError}=await client.from('scan_inzendingen').update({organisatie_id:tenantId,klant_slug:org.slug||null,company_key:companyKey,tenant_identity_status:'verified',bijgewerkt_op:new Date().toISOString()}).eq('id',scan.id).select('id,submission_key,organisatie_id,company_key,tenant_identity_status').single();
