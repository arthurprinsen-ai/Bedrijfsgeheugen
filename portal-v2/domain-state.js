...
function publicSnapshot(state,status,error){
 return Object.freeze({state:clone(state),status,error:error||null});
}
...
export function createDomainState({load,save}={}){
 ...
 const publish=()=>{
  const snap=publicSnapshot(state,currentStatus,currentError);
 ...
 function get(path=''){
  return path?readPath(state,path):clone(state);
 }

 function set(path,value){
  state=writePath(state,path,value);
  markDirty();
  return get(path);
 }

 function patch(path,value){
  if(!isObject(value))throw new TypeError('DOMAIN_STATE_PATCH_OBJECT_REQUIRED');
  state=writePath(state,path,value,{merge:true});
  markDirty();
  return get(path);
 }

 async function performFlush(){
  if(currentStatus!=='dirty'&&currentStatus!=='error')return publicSnapshot(state,currentStatus,currentError);
 ...
 return Object.freeze({
  ...
  error:()=>currentError,
  snapshot:()=>publicSnapshot(state,currentStatus,currentError),
  subscribe(listener){
   if(typeof listener!=='function')throw new TypeError('DOMAIN_STATE_SUBSCRIBER_REQUIRED');
...
export function createPortalDomainState(stateClient){
 ...
 return createDomainState({
  ...
  save:async nextState=>{
   const snap=await stateClient.write(nextState);
   if(snap?.mode!=='authenticated')throw new Error('PORTAL_STATE_CONFIRMATION_REQUIRED');
   return snap.state||{};
  }
 });
}
