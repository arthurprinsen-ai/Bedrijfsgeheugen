...
function state({growth=0,baseline=0,maturity=2}={}){
  return {portal:{profile:{maturity:{sturing:maturity,commercie:maturity,operatie:maturity}},market:{growth,digitalMaturity:baseline},strategicModels:{bcg:{note:''}}}};
}
...
test('BCG action reuses canonical root roadmap and deduplicates by id',()=>{
  const action=buildBcgRoadmapAction(buildBcgModel(state({growth:3,baseline:4,maturity:2})));
  const initial={...state({growth:3,baseline:4,maturity:2}),roadmap:[{id:'existing',title:'Bestaand'}]};
  const once=mergeBcgRoadmapAction(initial,action),twice=mergeBcgRoadmapAction(once,action);
...
test('a changed BCG quadrant replaces stale derived BCG advice',()=>{
