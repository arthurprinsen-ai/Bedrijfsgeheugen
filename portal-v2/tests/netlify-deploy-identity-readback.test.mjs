...
const releaseEvidenceBuilder=fs.readFileSync(new URL('../../tools/bouw-release-evidence.mjs',import.meta.url),'utf8');
...
test('Netlify build publishes exact commit identity for release readback',()=>{
  ...
  assert.match(builder,/commit_ref/);
  assert.match(releaseEvidenceBuilder,/release\.json/);
  assert.match(releaseEvidenceBuilder,/commit_ref/);
  assert.match(releaseEvidenceBuilder,/deploy_id/);
});
