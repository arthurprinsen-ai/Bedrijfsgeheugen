...
function state({ growth, own = 3, baseline = 3.2 } = {}) {
  return {
    portal: {
      profile: { maturity: { operatie: own, commercie: own, service: own, finance: own } },
...
test('BCG reproduces the exact legacy quadrant boundary rules', () => {
  ...
  // Legacy has no special baseline>0 exception: own >= baseline is strong even at baseline 0.
...
test('Vraagteken exposes a deterministic action for the canonical roadmap, not a parallel queue', () => {
  ...
  assert.deepEqual(model.roadmapAction, {
