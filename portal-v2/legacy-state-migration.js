...
function migrateRawLegacyCollections(upgraded,legacy){
 ...
 if(legacy.omzet!==undefined){
  setPath(upgraded,'portal.profile.revenue',numericOrRaw(legacy.omzet));
  if(!isObject(legacy.cijfers)||legacy.cijfers.cOmzet===undefined)setPath(upgraded,'portal.metrics.revenue',numericOrRaw(legacy.omzet));
 }
