...
function setBrand(name){
  ...
  localStorage.setItem(BRAND_KEY,clean);
  const brand=document.querySelector('.brand');if(brand)brand.lastChild.textContent=clean;
  document.documentElement.dataset.customerBrand=clean;
}
...
function ensureIdentity(){
  ...
  return new Promise(resolve=>{
    ...
    if(existing){existing.addEventListener('load',()=>resolve(window.netlifyIdentity||null),{once:true});return;}
    const script=document.createElement('script');script.src='https://identity.netlify.com/v1/netlify-identity-widget.js';script.async=true;script.dataset.v2Identity='true';
    script.addEventListener('load',()=>resolve(window.netlifyIdentity||null),{once:true});script.addEventListener('error',()=>resolve(null),{once:true});document.head.appendChild(script);
  });
}

function exportSnapshot(){
  const data={version:2,exportedAt:new Date().toISOString(),storage:{}};
  for(let i=0;i<localStorage.length;i++){const key=localStorage.key(i);if(key?.startsWith(STORAGE_PREFIX))data.storage[key]=localStorage.getItem(key)}
  const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`bedrijfsgeheugen-v2-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url);
}
...
function captureFeedback(){
  const text=prompt('Welke feedback wil je vastleggen?');if(!text?.trim())return;
  const items=safeParse(localStorage.getItem(FEEDBACK_KEY),'[]');const list=Array.isArray(items)?items:[];list.push({text:text.trim(),at:new Date().toISOString()});localStorage.setItem(FEEDBACK_KEY,JSON.stringify(list));alert('Feedback is vastgelegd in Portal V2.');
}
...
function mountUtilities(){
  const host=document.querySelector('.actionrow');if(!host||host.querySelector('[data-v2-utilities]'))return;
  const wrap=document.createElement('div');wrap.className='v2utilities';wrap.dataset.v2Utilities='true';
  const input=document.createElement('input');input.type='file';input.accept='application/json';input.hidden=true;input.addEventListener('change',()=>importSnapshot(input.files?.[0]));
  wrap.append(
    utilityButton('Export','export',exportSnapshot),
    utilityButton('Import','import',()=>input.click()),
    utilityButton('Print','print-permission',requestPrint),
    utilityButton('Feedback','feedback',captureFeedback),
    utilityButton('Klantmerk','customer-branding',changeBrand),
    utilityButton('Uitloggen','identity-login-logout',logout),input
  );host.appendChild(wrap);
}
function mountOverviewParity(){
  const anchor=document.querySelector('.kpis');if(!anchor||document.querySelector('[data-legacy-overview]'))return;
  const section=document.createElement('section');section.className='card legacyoverview';section.dataset.legacyOverview='true';section.innerHTML=`<div class="sectiontitle"><div><h3>Bedrijfsbeeld</h3><p>De volledige kernset uit het oorspronkelijke portaal, nu native in V2.</p></div><span>46 weken als conservatieve jaarbasis</span></div><div class="legacygrid">${OVERVIEW_CAPABILITIES.map(([id,label,value,detail])=>`<article data-overview-capability="${id}"><small>${label}</small><strong>${value}</strong><p>${detail}</p></article>`).join('')}</div>`;
  anchor.insertAdjacentElement('afterend',section);
}
function bindLegacyDeepLinks(openPage){
  const raw=new URLSearchParams(location.search).get('legacy')||location.hash.replace(/^#/,'');const target=LEGACY_CAPABILITY_MAP[raw];if(target)requestAnimationFrame(()=>openPage(target));
}

export function mountLegacyParity({openPage}){
  restoreBrand();mountUtilities();mountOverviewParity();bindLegacyDeepLinks(openPage);
  document.querySelector('.mobilebar')?.setAttribute('data-capability','mobile-navigation');
  ensureIdentity();
}
