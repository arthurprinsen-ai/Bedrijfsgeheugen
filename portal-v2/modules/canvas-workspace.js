...
export function mountCanvasWorkspace(root,{domainState,pageId='canvassen'}={}){
 ...
 const draw=()=>{
  ...
  root.innerHTML=`<section class="pvmodule canvas-parity" data-canvas-parity="full">
   ...
   <div class="v2workspacetabs" role="tablist">${TABS.map(([id,label],i)=>`<button type="button" role="tab" data-workspace-tab="${id}" aria-selected="${i===0?'true':'false'}">${label}</button>`).join('')}</div>
