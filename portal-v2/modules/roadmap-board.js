...
function cardMarkup(item){
 ...
 return `<article class="v2roadmapcard" draggable="true" data-repeat-row data-roadmap-id="${esc(item.id)}" data-sprint="${sprint}">
  ...
  <div class="v2roadmapmeta">
   ...
   <label>Eigenaar<input data-card-field="owner" data-repeat-col="owner" value="${esc(item.owner||'')}"></label>
  </div>
...
function boardMarkup(items){
 const lanes=Array.from({length:12},(_,index)=>index+1).map(sprint=>{
  const laneItems=items.filter(item=>clampSprint(item.sprint)===sprint);
  const cards=laneItems.map(cardMarkup).join('');
  return `<section class="v2roadmaplane" data-sprint="${sprint}" aria-label="Sprint ${sprint}"><header><b>Sprint ${sprint}</b><small>${laneItems.length} kaart(en)</small></header><div class="v2roadmapdrop" data-sprint="${sprint}">${cards||'<p class="v2roadmapempty">Sleep of verplaats een kaart hierheen</p>'}</div></section>`;
 }).join('');
 return `<div class="v2roadmaptoolbar"><div><strong>Uitvoeringsbord</strong><span>Sleep op desktop; gebruik pijlen op mobiel.</span></div><button type="button" data-add-roadmap data-repeat-add>+ Kaart toevoegen</button></div><div class="v2roadmapboard" data-roadmap-board>${lanes}</div>`;
}
