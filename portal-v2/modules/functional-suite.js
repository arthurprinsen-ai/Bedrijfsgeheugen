...
export const LEGACY_POLICY_NAMES=Object.freeze(['Informatiebeveiligingsbeleid','Toegangsbeleid en rechten','Incident-responseplan','Back-up en herstel — getest','Verwerkingsregister en bewaartermijnen','Verwerkersovereenkomsten','AI-gebruiksbeleid','Datadefinities en eigenaren (BI)','Rapportagekalender','Leveranciers- en ketenafspraken','Duurzaamheidsgegevens (CSRD)','Uitwijk en continuïteit']);
export const LEGACY_ESG_ITEMS=Object.freeze([{key:'energie',label:'Energieverbruik'},{key:'co2',label:'CO₂-uitstoot'},{key:'afval',label:'Afval en grondstoffen'},{key:'water',label:'Waterverbruik'},{key:'vervoer',label:'Vervoer en wagenpark'},{key:'arbo',label:'Arbeidsomstandigheden'},{key:'divers',label:'Diversiteit en gelijke beloning'},{key:'opleiding',label:'Opleiding en ontwikkeling'},{key:'keten',label:'Arbeid in de keten'},{key:'ethiek',label:'Bedrijfsethiek en anticorruptie'},{key:'bestuur',label:'Bestuur en verantwoordelijkheid'}]);
export const LEGACY_ESG_STATUS=Object.freeze([{value:'0',label:'Niet beschikbaar'},{value:'1',label:'Met de hand bij elkaar gezocht'},{value:'2',label:'Deels uit een systeem'},{value:'3',label:'Komt vanzelf uit de administratie'}]);
const aiCapabilities=AI_CAPABILITY_CATALOG.lagen.flatMap(layer=>layer.caps.map(cap=>({...cap,layer:layer.n})));
const executionCanvasSpec=[['ecNaam','Naam van de verandering','text'],['ecProj','Eerste project','text'],['ecThema','Activeer bij thema','text'],['ecAfd','Afdelingen die het raakt','text'],['ecProc','Processen','text'],['ecSys','Systemen','text'],['ecData','Gegevens','text'],['ecAi','AI-toepassingen','text'],['ecKpi','Maatstaven','text'],['ecGov','Governance en afspraken','text'],['ecDim','Bedrijfsonderdeel','text']];
...
const executionCanvasFields=executionCanvasSpec.map(([legacyId,label,type])=>f(`execution_${legacyId}`,legacyId,`Execution Canvas · ${label}`,type,`portal.canvases.execution.${legacyId}`,{required:false}));
...
const DEFINITIONS=Object.freeze({
 ...
 'cijfers-maatstaven':{slice:'portal.metrics',fields:[
   f('revenue','cOmzet','Omzet (€ x 1.000)','currency','portal.metrics.revenue',{min:0}),f('grossMargin','cBrutomarge','Brutomarge (%)','percentage','portal.metrics.grossMargin',{min:0,max:100}),f('ebitda','cEbitda','EBITDA (€ x 1.000)','currency','portal.metrics.ebitda'),f('wages','cLoon','Loonkosten (€ x 1.000)','currency','portal.metrics.wages',{min:0}),f('customers','cKlanten','Aantal klanten','number','portal.metrics.customers',{min:0}),f('largestCustomer','cGrootste','Grootste klant (%)','percentage','portal.metrics.largestCustomer',{min:0,max:100}),f('marketing','cMarketing','Marketing (€ x 1.000)','currency','portal.metrics.marketing',{min:0}),f('newCustomers','cNieuw','Nieuwe klanten','number','portal.metrics.newCustomers',{min:0}),f('dso','cDso','DSO (dagen)','number','portal.metrics.dso',{min:0}),f('it','cIt','IT-kosten (€ x 1.000)','currency','portal.metrics.it',{min:0}),f('nps','kNps','NPS','number','portal.metrics.nps',{min:-100,max:100}),f('satisfaction','kTevreden','Tevredenheid (1-10)','number','portal.metrics.satisfaction',{min:1,max:10,step:.1}),f('repeat','kHerhaal','Herhaalaankopen (%)','percentage','portal.metrics.repeat',{min:0,max:100}),f('complaints','kKlacht','Klachten','number','portal.metrics.complaints',{min:0}),f('measurementDate','mtDatum','Meetdatum','date','portal.metrics.measurement.date'),f('measurementType','mtSoort','Meting','text','portal.metrics.measurement.type'),f('measurementValue','mtWaarde','Waarde','number','portal.metrics.measurement.value'),f('measurementNote','mtNotitie','Notitie','textarea','portal.metrics.measurement.note'),f('billable','pDeclarabel','Declarabel of productief (%)','percentage','portal.metrics.performance.billable',{min:0,max:100}),f('onTime','pOtif','Op tijd geleverd (%)','percentage','portal.metrics.performance.onTime',{min:0,max:100}),f('leadTime','pDoorloop','Doorlooptijd (dagen)','number','portal.metrics.performance.leadTime',{min:0}),f('defects','pFout','Foutpercentage (%)','percentage','portal.metrics.performance.defects',{min:0,max:100}),f('quoteConversion','pOfferte','Offerteconversie (%)','percentage','portal.metrics.performance.quoteConversion',{min:0,max:100}),f('orders','pOrders','Orders per jaar','number','portal.metrics.performance.orders',{min:0}),f('training','pOpleiding','Opleiding (uur per medewerker)','number','portal.metrics.performance.training',{min:0}),f('churn','pVerloopKlant','Klantverloop (%)','percentage','portal.metrics.performance.churn',{min:0,max:100})],models:['KPI benchmark comparison','trusted-advisor ladder','productivity','measurements over time'],actions:[['Waarde & financiering','waarde-financiering'],['Branche','branche-markt']]},
 'waarde-financiering':{slice:'portal.valueFinance',fields:[f('debt','wSchuld','Schuld (€ x 1.000)','currency','portal.valueFinance.debt'),f('cash','wCash','Cash (€ x 1.000)','currency','portal.valueFinance.cash'),f('equity','wEV','Eigen vermogen (€ x 1.000)','currency','portal.valueFinance.equity'),f('balance','wBalans','Balanstotaal (€ x 1.000)','currency','portal.valueFinance.balance'),f('fixed','wVast','Vaste kosten (€ x 1.000)','currency','portal.valueFinance.fixed'),f('interest','wRente','Rentelasten (€ x 1.000)','currency','portal.valueFinance.interest'),f('multiple','wMultiple','EBITDA-multiple','number','portal.valueFinance.multiple',{min:1,max:15,step:.5,defaultValue:5}),f('wacc','wWacc','Vermogenskosten (WACC)','percentage','portal.valueFinance.wacc',{min:3,max:30,step:.5,defaultValue:10})],models:['EBITDA multiple','DCF perpetuity','DuPont','Altman Z','interest coverage','DSCR','break-even','sensitivity'],actions:[['Due diligence','due-diligence'],['Businesscase','businesscase']]},
 mensen:{slice:'portal.people',fields:[f('absence','mVerzuim','Verzuim (%)','percentage','portal.people.absence',{min:0,max:100}),f('turnover','mVerloop','Verloop (%)','percentage','portal.people.turnover',{min:0,max:100}),f('enps','mEnps','eNPS','number','portal.people.enps',{min:-100,max:100}),f('mto','mMto','Laatste MTO','select','portal.people.mto',{options:[{value:'0',label:'nooit gedaan'},{value:'1',label:'langer dan 2 jaar geleden'},{value:'2',label:'binnen 2 jaar'},{value:'3',label:'jaarlijks, met opvolging'}]}),f('vacancies','mVac','Openstaande vacatures','number','portal.people.vacancies',{min:0}),repeat('roles','peopleRoles:*','Rollen & kennisrisico','portal.people.roles',[col('name','Naam'),col('role','Rol'),col('criticalKnowledge','Kritieke kennis'),col('backup','Back-up')])],models:['people-vs-industry'],actions:[['Branche','branche-markt'],['Actueel houden','actueel-houden']]},
 ...
 onderzoek:{slice:'portal.research',fields:[f('filter','ondFilter:*','Onderzoeksfilter','text','portal.research.filter',{required:false}),f('projectFilter','pjFilter','Statusfilter','select','portal.research.projectFilter',{required:false,options:['alle','Draait','In aanbouw','Voorgesteld','Idee']}),f('projectNote','pjNotitie','Notitie bij het werk','textarea','portal.research.projectNote',{required:false}),repeat('hypotheses','research:*','Hypotheses & bewijs','portal.research.hypotheses',[col('hypothesis','Hypothese'),col('evidence','Bewijs'),col('source','Bron'),col('confidence','Zekerheid','range',{min:1,max:5}),col('reviewDate','Reviewdatum','date')])],models:['four-quadrant maturity/cost','cost-of-doing-nothing ladder','research evidence cards'],actions:[['Branche','branche-markt'],['Strategie','strategie-naar-maandagochtend']]},
 'compliance-governance':{slice:'portal.compliance',fields:[...LEGACY_POLICY_NAMES.map((label,index)=>f(`policy${index}`,`beleidLijst:${index}`,label,'select',`portal.compliance.policies.${index}`,{options:['ontbreekt','concept','vastgesteld','geoefend']})),...LEGACY_ESG_ITEMS.map((item,index)=>f(`esg${index}`,`esgVelden:${index}`,item.label,'select',`portal.compliance.esg.${index}`,{options:LEGACY_ESG_STATUS,help:`${item.key} · legacy CSRD-status 0–3`}))],models:['technology state','governance maturity','CSRD readiness','policy-document state','deadlines/fines','incident plan'],actions:[['AI-capabilities','ai-capabilities'],['Gegevens invullen','gegevens-invullen']]},
 'ai-capabilities':{slice:'portal.aiCapabilities',fields:aiCapabilities.map(cap=>f(cap.id,`aicap:${cap.id}`,`${cap.layer} · ${cap.n}`,'range',`portal.aiCapabilities.${cap.id}`,{min:1,max:5,required:false,help:cap.d})),models:['AI capability maturity','nine-layer capability architecture','scan provenance','relevance timing'],actions:[['Data & AI','data-ai'],['Strategy DNA','strategy-dna']]},
 'strategie-naar-maandagochtend':{slice:'portal.strategy',fields:[f('horizon','kHorizon','Binnen hoeveel maanden','range','portal.strategy.horizon',{min:1,max:12,step:1,defaultValue:12}),f('minimumValue','kMin','Minimale opbrengst (€)','range','portal.strategy.minimumValue',{min:0,max:30000,step:1000,defaultValue:0}),repeat('findings','strategyFindings:*','Strategische bevindingen','portal.strategy.findings',[col('finding','Bevinding'),col('model','Model'),col('value','Waarde (€)','currency'),col('horizon','Horizon'),col('owner','Eigenaar')])],models:['all-model findings matrix','strategy conclusion','filtered recommendation map'],actions:[['Canvassen','canvassen'],['Eindconclusie','eindconclusie'],['Roadmap','roadmap']]},
 canvassen:{slice:'portal.canvases',fields:[...canvasNames.flatMap(([key,label])=>[f(`${key}question`,key,`${label} · kernvraag`,'textarea',`portal.canvases.${key}.answer`,{required:false}),f(`${key}owner`,`${key}:owner`,`${label} · eigenaar`,'owner',`portal.canvases.${key}.owner`,{required:false})]),...executionCanvasFields],models:['Business Model Canvas','Waardepropositie','Lean Canvas','Merkcanvas','Contentcanvas','Salescanvas','Execution Canvas','canvas conclusion'],actions:[['Strategie','strategie-naar-maandagochtend'],['Eindconclusie','eindconclusie']]},
 ...
 wijzigingen:{slice:'portal.changes',fields:[repeat('changes','wijzigingen:*','Wijzigingen','portal.changes.items',[col('change','Wijziging'),col('area','Onderdeel'),col('impact','Impact','range',{min:1,max:5}),col('owner','Eigenaar'),col('status','Opvolging','select',{options:['Open','Bezig','Geborgd']})]),f('taskTheme','twThema','Van wijziging naar taken · wat verandert er','text','portal.changes.toTasks.theme',{required:false}),f('taskDue','twWanneer','Van wijziging naar taken · klaar voor','date','portal.changes.toTasks.due',{required:false}),repeat('tasks','twTaken:*','Taken uit deze wijziging','portal.tasks.items',[col('title','Taak'),col('department','Afdeling'),col('owner','Eigenaar'),col('due','Deadline','date'),col('status','Status','select',{options:['Open','Bezig','Geblokkeerd','Klaar']})])],models:['change history','impact by department','change-to-tasks'],actions:[['Actueel houden','actueel-houden'],['Roadmap','roadmap']]},
 ...
 offerte:{slice:'portal.offer',fields:[f('package','offerte:pakket','Pakket','select','portal.offer.package',{options:['Start','Scale','Control','Enterprise']}),f('scope','offerte:scope','Scope','textarea','portal.offer.scope'),f('sprints','offerte:sprints','Sprints','number','portal.offer.sprints',{min:1,max:24}),f('weeklyPrice','offerte:week','Prijs per week (€)','currency','portal.offer.weeklyPrice',{min:0}),f('startDate','offerte:start','Gewenste start','date','portal.offer.startDate'),f('approvalName','htkNaam','Akkoord · je naam','text','portal.offer.approval.name',{required:false}),f('approvalRole','htkFunctie','Akkoord · je functie','text','portal.offer.approval.role',{required:false}),f('approved','htkAkkoord','Ik geef akkoord op de gekozen onderdelen en de vaste prijs','triState','portal.offer.approval.agreed',{required:false}),f('approvalDate','htkVinkje','Datum akkoord','date','portal.offer.approval.date',{required:false}),repeat('additionalWork','mwRijen:*','Meerwerk','portal.offer.additionalWork',[col('description','Wat'),col('reason','Waarom'),col('hours','Uren','number',{min:0}),col('price','Prijs (€)','currency'),col('requestedBy','Aangevraagd door'),col('status','Status','select',{options:['Voorgesteld','Akkoord','Afgewezen']}),col('approvedBy','Akkoord door')])],models:['package/sprint offer','pricing','delivery story','acceptance/start flow'],actions:[['Advies','advies'],['Roadmap','roadmap']]},
