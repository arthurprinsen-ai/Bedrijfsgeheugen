...
.legacy-donut-row{display:grid;grid-template-columns:minmax(160px,220px) 1fr;gap:16px;align-items:center}.legacy-donut{aspect-ratio:1;border-radius:50%;position:relative;background:conic-gradient(#315be8 0 13%,#efa800 13% 25%,#229c5a 25% 37%,#7157d9 37% 49%,#48c3bb 49% 61%,#8ea1c9 61% 73%,#da6b56 73% 85%,#9bb04b 85% 100%)}.legacy-donut:after{content:'';position:absolute;inset:27%;background:#fff;border-radius:50%}.legacy-donut-center{position:absolute;inset:0;display:grid;place-content:center;text-align:center;z-index:1}.legacy-donut-center strong{font-size:28px}.legacy-donut-center span{font-size:11px;color:#6d7486}
...
export function legacyOverviewCompleteModel(state={}){
 ...
 return Object.freeze({costs,total,top,avg,cmmi,stage:STAGES[cmmi-1],sharpness:legacySharpnessModel(state),roadmap:r,advice:advice(state).slice(0,5)});
}
...
export function renderLegacyOverviewComplete(root,state={},openPage=()=>{}){
 ...
 <div class="legacy-grid2">
  ...
  <article class="legacy-card"><div class="legacy-kicker"><small>Waar je staat op de adoptiecurve</small><strong>${esc(m.stage?.segment||'Nog niet bepaald')}</strong></div><p>${esc(m.stage?.name||'Vul je profiel in')}</p><div class="legacy-state-track">${adoptionMarkup(m)}</div></article>
 </div>
 <div class="legacy-grid2">
  <article class="legacy-card"><h3>Waar de tijd weglekt</h3><p>Capaciteitswaarde per onderdeel op basis van uren × niveau × medewerkers × 46 weken × uurkosten.</p><div class="legacy-donut-row"><div class="legacy-donut"><div class="legacy-donut-center"><strong>${euro(m.total)}</strong><span>per jaar</span></div></div><div class="legacy-cost-list">${m.costs.slice(0,8).map(x=>`<div class="legacy-cost-row"><button type="button" data-legacy-page="businesscase"><span><b>${esc(x.label)}</b><br><small>${esc(LEVEL_LABELS[x.niveau]||'onbekend')}</small></span><strong>${euro(x.kosten)}</strong></button></div>`).join('')||'<p>Nog onvoldoende gegevens voor een bedrag.</p>'}</div></div></article>
