...
const nl = (value, digits=1) => Number(value).toLocaleString('nl-NL',{maximumFractionDigits:digits});
...
export function withResourceFootprint(footprint={}, base=DEFAULT_IMPACT_SNAPSHOT){
  ...
  const dataBacked=coverage>0;
  if(!dataBacked) return structuredClone(base);
  const calculatedAt=footprint.calculatedAt||new Date().toISOString();
  return {
    ...structuredClone(base),
    realtime:[
      ['⚡',`${nl(footprint.energyKwh)} kWh`,'energie · berekend uit bronverbruik'],
      ['☁',`${nl(footprint.co2eKg)} kg`,'CO₂e · factor-gebaseerd'],
      ['●',`${nl(footprint.waterLiters,0)} liter`,'waterimpact · factor-gebaseerd'],
    ...
    resourceFootprint:{
      ...
      confidence,
      calculatedAt,
      factorVersions:[...(footprint.factorVersions||[])],
      methodologies:[...(footprint.methodologies||[])],
      measurementClass:footprint.measurementClass||'mixed'
    },
    internal:{
      ...(base.internal||{}),
      dataQuality:`${Math.round(coverage*100)}%`,
      lastValidation:new Date(calculatedAt).toLocaleString('nl-NL'),
      footprintConfidence:`${Math.round(confidence*100)}%`
    }
  };
}
...
function mobileDomainCard([id,m]){
  const descriptions={climate:'Uitstoot daalt, maar ketenemissies en bewijs blijven de volgende focus.',water:'Verbruik per FTE verbetert; maak locaties en brondata auditbaar.',circularity:'Circulariteit ligt op koers; sluit de resterende materiaalstromen.',social:'Medewerkerimpact is sterk; borg trend, diversiteit en inzet met evidence.',governance:'Governance staat op koers; houd controls en rapportage aantoonbaar actueel.'};
...
export function csrdImpactMarkup(snapshot=DEFAULT_IMPACT_SNAPSHOT,{customerView=false}={}){
  ...
  const realtimeStatus=footprint?.dataBacked?`Data-backed · ${Math.round(footprint.coverage*100)}% brondekking`:'Voorbeelddata · geen live claim';
  ...
  return `<div class="csrd-cockpit" data-customer-view="${customerView}">
    ...
    <section class="csrd-mobile-summary" aria-label="Mobiele CSRD samenvatting"><article><small>Totale impactscore</small><strong>${data.impactScore}/100</strong><span>↑ +${data.impactDelta} t.o.v. vorig jaar</span></article><article><small>CSRD readiness</small><strong>${data.readiness}%</strong><span>Rapportagegereedheid</span></article><article><small>Prioriteit</small><strong>${data.actions.filter(([, ,prio])=>prio==='Hoog').length} hoog</strong><span>Directe acties</span></article></section>
    <section class="csrd-stage">
      ...
      <div class="csrd-world"><div class="csrd-sky"></div><div class="csrd-sun"></div><div class="csrd-hills"></div><div class="csrd-city"></div><div class="csrd-river"></div><div class="csrd-wind w1">✣</div><div class="csrd-wind w2">✣</div><div class="csrd-solar">▦ ▦ ▦</div><div class="csrd-building"><span>BEDRIJFSGEHEUGEN</span></div><div class="csrd-worldcopy"><strong>Je impact in één oogopslag.</strong><br><small>Van klimaat en water tot social, governance en bewijs.</small></div>${Object.entries(data.metrics).map(metricCard).join('')}<div class="csrd-orbit">PEOPLE <b>+</b> PLANET <b>+</b> PROGRESS</div></div>
      <aside class="csrd-side"><article class="csrd-panel readiness"><div class="csrd-panelhead"><h3>CSRD Readiness</h3><span>readiness-overzicht</span></div><div class="csrd-readyrow">${meter(data.readiness,'','%')}<p>Op weg naar volledige rapportage</p></div><ul>${readiness(data)}</ul><button type="button" data-csrd-open="audit">Bekijk details →</button></article><article class="csrd-panel"><div class="csrd-panelhead"><h3>Impact in real time</h3><span>${realtimeStatus}</span></div><div class="csrd-live">${data.realtime.map(([ic,v,s])=>`<div><i>${ic}</i><span><b>${v}</b><small>${s}</small></span></div>`).join('')}</div></article></aside>
    </section>
    ...
    <section class="csrd-grid"><article class="csrd-panel domain-card" data-domain="climate"><h3>CO₂-uitstoot <small>(ton CO₂e)</small></h3>${sparkBars([84,65,49,37,31])}<footer><span>2021</span><span>2022</span><span>2023</span><span>2024</span><b>Doel 2030</b></footer></article><article class="csrd-panel domain-card" data-domain="water"><h3>Waterverbruik <small>(m³)</small></h3>${sparkBars([90,71,54,39,28])}<footer><span>2021</span><span>2022</span><span>2023</span><span>2024</span><b>Doel 2030</b></footer></article><article class="csrd-panel domain-card social-card" data-domain="social"><h3>Social impact</h3>${data.social.map(([l,v,d])=>`<div><span>${l}</span><b>${v}</b><em class="${d.startsWith('-')?'down':''}">${d}</em></div>`).join('')}</article><article class="csrd-panel domain-card circular-card" data-domain="circularity"><h3>Circulariteit</h3><div class="csrd-circle"><strong>78%</strong></div><ul><li>Herbruikbaar</li><li>Recycling</li><li>Biobased</li><li>Nog te gaan</li></ul></article></section>
    ...
    ${customerView?'':`<section class="csrd-evidence"><b>Datakwaliteit ${data.internal.dataQuality}</b><span>${data.internal.openEvidence} open evidence-items</span><span>Laatst gevalideerd ${data.internal.lastValidation}</span>${footprintEvidence}<button type="button" data-csrd-open="outcomes-evidence">Evidence & audittrail →</button></section>`}
  </div>`;
}
...
export function renderCsrdImpact(root,{openPage=()=>{},closePage=()=>{},snapshot=DEFAULT_IMPACT_SNAPSHOT}={}){
  ...
  const render=()=>{
    ...
    root.querySelectorAll('[data-csrd-focus]').forEach(btn=>btn.addEventListener('click',()=>{const domain=btn.dataset.csrdFocus;const tab=root.querySelector(`[data-csrd-tab="${domain}"]`);tab?.click();tab?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});}));
    root.querySelectorAll('[data-csrd-tab]').forEach(btn=>btn.addEventListener('click',()=>{root.querySelectorAll('[data-csrd-tab]').forEach(x=>x.classList.toggle('active',x===btn));const domain=btn.dataset.csrdTab;root.querySelectorAll('[data-domain]').forEach(card=>card.classList.toggle('csrd-muted',domain!=='all'&&card.dataset.domain!==domain));}));
  };
  render();
}
