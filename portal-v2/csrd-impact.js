...
function mobileDomainCard([id,m]){const descriptions={climate:'Uitstoot daalt, maar ketenemissies en bewijs blijven de volgende focus.',water:'Verbruik per FTE verbetert; maak locaties en brondata auditbaar.',circularity:'Circulariteit ligt op koers; sluit de resterende materiaalstromen.',social:'Medewerkerimpact is sterk; borg trend, diversiteit en inzet met evidence.',governance:'Governance staat op koers; houd controls en rapportage aantoonbaar actueel.'};return `<article class="csrd-mobile-domain" data-domain="${id}"><div class="domain-head"><span class="domain-icon">${m.icon}</span><div><small>${m.label}</small><strong>${m.value}</strong><small>${m.sub}</small></div></div><p>${descriptions[id]}</p><button type="button" data-csrd-focus="${id}">Bekijk ${m.label.toLowerCase()} →</button></article>`;}
