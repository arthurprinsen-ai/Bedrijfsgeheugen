...
const problemContextItems=state=>Array.isArray(state?.powerhouse?.executive?.problems)?state.powerhouse.executive.problems.filter(p=>/^PH-P\d{3}$/.test(String(p?.problem_id||p?.problemId||''))).slice(0,5):[];
function renderProblemContext(state,context){
 const problems=problemContextItems(state);if(!problems.length)return '';
 const relevant=problems.filter(p=>{
  const surfaces=Array.isArray(p.context_surfaces)?p.context_surfaces:[];
  return !surfaces.length||surfaces.includes(context)||surfaces.includes('all');
 });
 if(!relevant.length)return '';
 return `<section class="os-context-strip" data-context-surface="${esc(context)}"><div class="os-context-strip-head"><strong>Relevante Probleemradar-context</strong><small>Dezelfde PH-Pxxx waarheid, contextueel getoond — externe signalen blijven context totdat tenant-evidence ze bevestigt.</small></div><div class="os-context-strip-grid">${relevant.slice(0,3).map(p=>`<article><div><span>${esc(p.problem_id||p.problemId)}</span><em>${esc(String(p.impact_label||p.impactLabel||'POTENTIAL').toUpperCase())}</em></div><strong>${esc(p.title||p.name||p.problem_name||p.problem_id)}</strong>${p.buying_trigger?`<small>Trigger: ${esc(p.buying_trigger)}</small>`:''}${p.context_relevance||p.benchmark_context?`<p>${esc(p.context_relevance||p.benchmark_context)}</p>`:''}</article>`).join('')}</div></section>`;
}
...
function renderImpact(state){
 ...
 return `${renderProblemContext(state,'impact-engine')}<section class="os-page-card"><h3>€ Impact Engine</h3><div class="os-impact-grid">${cards.join('')}</div></section>`;
}
...
function renderDecisions(state){
 const actions=state?.powerhouse?.executive?.next_best_actions||[];return `${renderProblemContext(state,'next-best-actions')}${items('Next Best Actions & besluiten',actions,item=>{const risk=classifyActionRisk(item);const eligibility=evaluateActionEligibility({...item,risk_class:risk},{approved:false,policy_allowed:false});return `<div><strong>${esc(item.title||item.id)}</strong><p>${esc(item.explanation||'')}</p><small>Risicoklasse ${risk} · ${esc(eligibility.state)}</small><div class="os-actions">${DECISIONS.map(d=>`<button type="button" data-os-decision="${esc(d)}" data-action-id="${esc(item.id)}">${esc(d)}</button>`).join('')}</div></div>`;})}`;
}
...
function renderMonitoring(state){const signals=dedupeSignals(state?.powerhouse?.signals||[]);return `${renderProblemContext(state,'monitoring-learning')}${items('Continuous Monitoring',signals,item=>`<div><strong>${esc(item.title||item.type)}</strong><p>${esc(item.summary||item.entity_id||'')}</p><small>Δ ${esc(item.delta)} · confidence ${Math.round((Number(item.confidence)||0)*100)}%</small></div>`)}`;}
...
function renderEvidence(state){const sources=state?.powerhouse?.sources||[];return `${renderProblemContext(state,'evidence-health')}${items('Data & Evidence Health',sources,s=>`<div><strong>${esc(s.name||s.id)}</strong><p>${esc(s.status||'onbekend')}</p><small>${esc(s.last_successful_sync||'geen syncbewijs')} · confidence ${Math.round((Number(s.confidence)||0)*100)}%</small></div>`)}`;}
