...
export function renderStrategyDna(container,{openPage,domainState=globalThis.__BG_PORTAL_DOMAIN_STATE__}={}){
 const state=domainState?.get?.()||{};const strategy=state?.portal?.strategy||{};const dna=strategy?.dna||{};const findings=Array.isArray(strategy?.findings)?strategy.findings:[];const execution=strategy?.execution||{};const filled=Object.entries(dna).filter(([key,value])=>key!=='updatedAt'&&String(value||'').trim()).length;const themeIds=Array.isArray(execution?.themeIds)?execution.themeIds:[];
