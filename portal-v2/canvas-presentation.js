...
function p(state){return state?.portal||{}}
function profile(state){return p(state).profile||{}}
function metrics(state){return p(state).metrics||{}}
function level(state,key){const pr=profile(state);return n((pr.maturity||pr.dimensions||{})[key],2)}
function answer(state,id){return String(p(state).canvases?.[id]?.answer||'')}
function owner(state,id){return String(p(state).canvases?.[id]?.owner||'')}
function branch(state){return p(state).market?.industry||profile(state).industry||profile(state).branch||'de gekozen branche'}
...
function annualManualCost(state){const pr=profile(state);return n(pr.manualHoursPerWeek)*46*n(pr.hourlyCost)}
...
function sectionsFor(id,state){
 const pr=profile(state),m=metrics(state),head=n(pr.headcount),rev=n(m.revenue)*1000,largest=n(m.largestCustomer),cost=annualManualCost(state),weak=weakest(state),ind=branch(state);
 const perEmployee=head&&rev?eur(rev/head):'Nog niet berekenbaar';
 const common={head,rev,largest,cost,weak,ind,perEmployee};
 const map={
  bmc:[['Klantsegmenten',answer(state,'bmc')||`${ind}; ${head||'onbekend aantal'} medewerkers als huidige context`],['Waardepropositie',`Maak ${weak} aantoonbaar eenvoudiger en voorspelbaarder`],['Kanalen','Website, relaties, sales en bestaande klantcontacten'],['Klantrelaties',largest>=25?`Concentratierisico: grootste klant ${pct(largest)}`:'Relatiebasis is niet sterk geconcentreerd'],['Inkomstenstromen',rev?`${eur(rev)} omzet uit huidige bedrijfsdata`:'Omzet nog niet ingevuld'],['Kernmiddelen',`Mensen, kennis, systemen en data; zwakste schakel ${weak}`],['Kernactiviteiten','Leveren, opvolgen, verbeteren en kennis borgen'],['Partners','Leveranciers en platforms die kritieke processen ondersteunen'],['Kostenstructuur',cost?`${eur(cost)} berekend handwerk per jaar`:'Handwerkkosten nog niet berekenbaar']],
