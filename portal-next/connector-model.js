...
const SUPPORTED_TRANSFORMS=new Set(['none','trim','lowercase','uppercase','constant','default']);
...
export function createConnectorDraft(templateId='blank'){
  ...
  return {
    id:null,version:1,name:draft.name,templateId:draft.id,state:'Draft',source:draft.source,
    documentSchema:draft.documentSchema,lookups:draft.lookups||[],validationRules:draft.validationRules||[],
    mappings:draft.mappings||[],target:draft.target,reviewPolicy:draft.reviewPolicy||{requiredBelowConfidence:.8},
    dedupe:draft.dedupe||{strategy:'content-hash'},runtime:{configured:false,evidence:null},legacy:draft.legacy||null
  };
}
...
export function normalizeConnectorDraft(input){
  ...
  draft.source=draft.source||{type:'upload',config:{}};draft.source.config=draft.source.config||{};
  ...
  draft.validationRules=Array.isArray(draft.validationRules)?draft.validationRules:[];
  draft.mappings=Array.isArray(draft.mappings)?draft.mappings:[];
  draft.target=draft.target||{type:'datahub',config:{}};draft.target.config=draft.target.config||{};
  draft.reviewPolicy=draft.reviewPolicy||{requiredBelowConfidence:.8};draft.dedupe=draft.dedupe||{strategy:'content-hash'};
  draft.runtime=draft.runtime||{configured:false,evidence:null};
  return draft;
}

export function validateConnectorDraft(input){
  const draft=normalizeConnectorDraft(input),errors=[];
  ...
  for(const [index,f] of draft.documentSchema.fields.entries()){
    ...
    else if(seen.has(key))add('DUPLICATE_FIELD_KEY',`documentSchema.fields.${index}.key`,`Dubbele veldsleutel: ${key}`);else seen.add(key);
  }
  for(const path of [...walkSecretKeys(draft.source.config,'source.config'),...walkSecretKeys(draft.target.config,'target.config'),...walkSecretKeys(draft.lookups,'lookups')])add('CLIENT_SECRET_FORBIDDEN',path,'Secrets horen uitsluitend server-side.');
...
export function activationEligibility(input,evidence){
  ...
  const complete=Boolean(evidence.configVersion===draft.version&&evidence.testExecutionId&&evidence.sourceReadSuccess===true&&evidence.extractionResult?.ok===true&&evidence.validationResult?.ok===true&&evidence.targetSafeTestResult?.ok===true);
