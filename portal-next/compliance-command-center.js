...
import { mergeBedrijfsgeheugenComplianceEvidence } from './bedrijfsgeheugen-compliance-evidence.js';
...
function riskCards(risks){
  ...
  return risks.slice(0,3).map(risk=>`<article class="cc-risk cc-risk--${esc(risk.severity||'medium')}" data-risk-id="${esc(risk.id)}">
...
function evidenceMarkup(c){
  ...
  if(!items.length)return '<span class="cc-evidence-tag">Geen bewijs</span>';
  return `<details class="cc-evidence-details"><summary class="cc-evidence-tag">${items.length} bewijsitem(s)</summary><ul>${items.map(item=>`<li>${item.href?`<a href="${esc(item.href)}" target="_blank" rel="noopener noreferrer">${esc(item.label||item.id)}</a>`:esc(item.label||item.id)}${item.detail?`<small>${esc(item.detail)}</small>`:''}</li>`).join('')}</ul></details>`;
}
...
<td><b>${esc(c.id)}</b></td><td><b>${esc(c.requirement)}</b><small>${frameworkLabel(c.framework)}</small></td><td>${frameworkLabel(c.framework)}</td><td><span class="cc-status tone-${STATUS_TONE[c.status]||'unknown'}">● ${STATUS_LABEL[c.status]||c.status}</span></td><td>${evidenceMarkup(c)}</td><td>${esc(c.owner||'Nog toewijzen')}</td><td>${esc(c.nextAction||'Beoordelen')}</td><td>${c.verifiedAt?fmtDate(c.verifiedAt):'—'}</td></tr>`).join('')}

function selfEvidencePanel(controls){
  const existing=controls.filter(c=>c.control?.implemented||c.evidence?.length);
  if(!existing.length)return '';
  return `<section class="cc-panel cc-self-evidence" data-compliance-self-evidence><div class="cc-panel-head"><div><h2>✓ Wat Bedrijfsgeheugen al aantoonbaar doet</h2><p>Feitelijke technische en governance-controls uit onze eigen repository, CI en productie-evidence. Onbekende wettelijke toepasselijkheid blijft bewust onbekend.</p></div></div><div class="cc-self-evidence-grid">${existing.map(c=>`<article><span class="cc-status tone-${STATUS_TONE[c.status]||'unknown'}">● ${STATUS_LABEL[c.status]||c.status}</span><h3>${esc(c.requirement)}</h3><p>${esc(c.reason||'')}</p>${evidenceMarkup(c)}</article>`).join('')}</div></section>`;
}
...
export class ComplianceCommandCenter extends (globalThis.HTMLElement||class{}){
 ...
 controls(){return this.scope==='bedrijfsgeheugen'?buildBedrijfsgeheugenControls(mergeBedrijfsgeheugenComplianceEvidence(globalThis.BG_SELF_COMPLIANCE_EVIDENCE||{})):buildCustomerControls(readPortalComplianceInput(globalThis))}
