...
**Role:** learning/prevention child only — not a second Portal V2 state  
**Status:** VERIFIED  
**Date:** 2026-09-17  
...
The initial risk was to interpret that request as a greenfield rebuild. Current-state inspection showed that this would have violated EXISTING-STATE-FIRST and REUSE-FIRST: the repository already contained a protected legacy baseline, a complete 24-capability functional inventory, executable parity ownership, persistence contracts, fail-closed tests and production evidence gates. Powerhouse also already contained canonical CurrentState `portal-v2-full-legacy-parity-v1`; this document therefore records a reusable learning beneath that state and never competes with it.

## Canonical authorities

- Canonical Portal V2 CurrentState: `portal-v2-full-legacy-parity-v1`.
...
- Human/read-model documentation: Powerhouse Latest Verified State, Powerhouse Menselijk Handboek and Powerhouse Direct Knowledge Base in Notion.
...
1. **Inspect before building.** Before any legacy-to-V2 restoration, read current protected `main`, canonical state `portal-v2-full-legacy-parity-v1`, `legacy-functional-inventory.js`, `parity-gate.js`, capability contracts, owner modules and current production evidence.
...
8. **Write back the learning without forking state.** Every material parity finding/fix records root cause, fix, evidence and prevention in the existing Powerhouse learning/documentation chain while preserving `portal-v2-full-legacy-parity-v1` as the single current-state authority.
...
- a second portal data authority, duplicate CurrentState or duplicated Powerhouse logic is introduced.

## Required recovery flow

`detect → reproduce → add/strengthen regression check → root-cause fix in existing owner → full Portal V2 tests → protected delivery gates → exact-SHA merge/promotion → production browser/runtime readback → canonical CurrentState refresh → Powerhouse learning/writeback`

No workaround may weaken parity, security, truth, RLS, persistence or production-evidence gates.

## Open obligation

None for parent state `portal-v2-full-legacy-parity-v1` or learning fingerprint `portal-v2-legacy-powerhouse-parity-v1` at the time of this writeback.
