...
export default async function handler(request,context){
  ...
  const [datahub,powerhouseCore,legacy]=await Promise.all([attemptDataHub(datahubEvent),attemptPowerhouseCore(observation,input),attemptLegacyBg211(envelope)]);
  const state=powerhouseCore.persisted?'powerhouse-core-persisted':datahub.persisted?'datahub-persisted':'degraded';
  await store.setJSON(key,{state,queued_at:queuedAt,last_attempt_at:new Date().toISOString(),attempts:(previous?.attempts||0)+1,datahub,powerhouse_core:powerhouseCore,legacy,envelope,datahub_event:datahubEvent});
  return json({accepted:true,deduped:false,event_id:envelope.event_id,datahub:datahub.persisted?'persisted':'queued',powerhouse_core:powerhouseCore.persisted?'persisted':'degraded',legacy:legacy.delivered?'delivered':'off'},powerhouseCore.persisted?202:503);
}
