...
async function attemptDataHub(outcome){
  ...
  try{const response=await fetch(`${base}/functions/v1/growth-datahub-ingest`,{method:'POST',headers:{'content-type':'application/json','x-bg-service-token':token},body:JSON.stringify({action:'outcome',outcome}),signal:AbortSignal.timeout(3500)});const body=await response.json().catch(()=>null);return {attempted:true,persisted:response.ok,status:response.status,reason:response.ok?'stored':'http-error',readback:response.ok?body?.result:null};}
...
async function attemptBg211(envelope){
  const url=env('BG211_WEBHOOK_URL');if(!url||env('BG211_DELIVERY_ENABLED')!=='true')return {attempted:false,delivered:false,reason:'delivery-disabled'};
  try{const response=await fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({event_json:JSON.stringify(envelope)}),signal:AbortSignal.timeout(3500)});return {attempted:true,delivered:response.ok,status:response.status,reason:response.ok?'accepted':'http-error'};}
...
export default async function handler(request,context){
  ...
  const datahub=await attemptDataHub(outcome);const delivery=await attemptBg211(envelope);const state=delivery.delivered?'delivered':datahub.persisted?'datahub-persisted':'queued';
  await store.setJSON(key,{state,queued_at:queuedAt,datahub,delivery,outcome,envelope,last_attempt_at:(datahub.attempted||delivery.attempted)?new Date().toISOString():null});
