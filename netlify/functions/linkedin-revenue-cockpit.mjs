...
function canonicalAction(x,notionByUrl=new Map()){
  ...
  return{
    ...
    buyingTriggerStatus:s(x.buying_trigger_status??x.evidence?.buying_trigger_status),
    buyingTriggers:a(x.buying_triggers??x.evidence?.buying_triggers),
    problemHypotheses:a(x.problem_hypotheses??x.evidence?.problem_hypotheses),
    recommendedOffers:a(x.recommended_offers??x.evidence?.recommended_offers),
    partnerChannels:a(x.partner_channels??x.evidence?.partner_channels),
    outreachMode:s(x.outreach_mode??x.evidence?.outreach_mode)||'context-led',
