...
export function normalizeOutcomeEnvelope(input){
  const raw=input?.body?.result??input?.result??input;
...
export function createSocialOutcomeHandler({persistEvent=persistBrainEvent,store}={}){
  return async request=>{
    ...
    try{
      ...
      return Response.json({error:status===409?'SOCIAL_OUTCOME_CONFLICT':'SOCIAL_OUTCOME_PERSISTENCE_FAILED',message:error.message},{status,headers:{'cache-control':'no-store'}});
    }
  };
}
