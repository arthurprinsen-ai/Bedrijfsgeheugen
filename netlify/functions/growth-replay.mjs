...
export default async function handler(request,context){
  ...
  for(const storeName of ['bg-growth-events','bg-growth-outcomes']){
    const store=getStore(storeName);const {blobs}=await store.list();
    for(const blob of blobs){
      if(replayed>=MAX_REPLAY)break;
      const key=blob.key;const record=await store.get(key,{type:'json'}).catch(()=>null);if(!record?.envelope||record?.delivery?.delivered||record?.state==='delivered')continue;
