...
function envConfig(){
  return {
    ...
    serviceToken:process.env.BG_SOCIAL_LEARNING_SERVICE_TOKEN||process.env.BG_PORTAL_EU_SERVICE_TOKEN||null,
    lookbackDays:Number(process.env.BUFFER_LOOKBACK_DAYS||8),
  };
}

async function recordCredentialObligation(store,now){
  if(!store?.recordObligation)return;
  await store.recordObligation({
    tenantId:'canonical',
    id:'buffer-social-learning:credential',
    type:'MISSING_CREDENTIAL',
    owner:'POWERHOUSE_SOCIAL_LEARNING',
    dueAt:new Date(now).toISOString(),
    status:'OPEN',
    source:'buffer',
    requiredSecret:'BUFFER_API_KEY',
  });
}
...
export async function runBufferCollection({
  ...
  if(!apiKey){
    await recordCredentialObligation(activeStore,now);
  ...
  if(!activeIngest){
    ...
    activeIngest=async envelope=>{
      ...
      if(!response.ok&&response.status!==409){
        ...
        throw new Error(`BUFFER_SOCIAL_OUTCOME_INGEST_${response.status}:${body}`);
      }
    };
  }
  const result=await collectBufferPosts({
    ...
    channelIds:scope.channels.map(channel=>channel.id),
    channelServices:services,
    tenantId,
    fetchFn,
    ingest:activeIngest,
    now,
    since:isoSince(now,lookbackDays),
  });
  return {ok:true,organizationId:scope.organizationId,channels:scope.channels.map(channel=>({id:channel.id,service:channel.service})),...result};
}

export default async function bufferCollect(input={}){
  const dependencyInjection=input&&typeof input==='object'&&!(input instanceof Request)&&('apiKey' in input||'store' in input||'ingest' in input);
