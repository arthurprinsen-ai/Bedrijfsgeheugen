...
export function bufferEnvelopeToCoreEvents(envelope={}){
  ...
  const common={source:'buffer',channel:envelope.platform,contentKey:envelope.contentHash||envelope.postId,topicKey,postUrl:envelope.url||'',text:envelope.text||'',occurredAt:envelope.publishedAt||envelope.observedAt,dataQuality:envelope.dataQuality||'OBSERVED'};
  const published={...common,eventType:'social_post_published',dedupeKey:`buffer:published:${envelope.postId}`,externalPostId:envelope.externalPostId,evidence:{publishedAt:envelope.publishedAt,channelId:envelope.channelId}};
  const metric={...common,eventType:'social_metric_observed',occurredAt:envelope.observedAt,dedupeKey:envelope.idempotencyKey,metrics:envelope.metrics||{},evidence:{metrics:envelope.metrics||{},observedAt:envelope.observedAt,externalPostId:envelope.externalPostId}};
