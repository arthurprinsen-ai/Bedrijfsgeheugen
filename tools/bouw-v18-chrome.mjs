...
import { TABEL_CSS, INTERACTIE_CSS, INTERACTIE_JS, verrijkInhoud, opDezePagina, mensenblok, schemas, ORGANISATIE_SCHEMA, EXTRA_HTML, volgendeStap, VOLGENDE_CSS , GEGEVENS, LOCATIE_SCHEMA} from './v18-verrijking.mjs';
  ...
  while (i < css.length) {
    ...
    if (isAt) {
      ...
      const selectors = kop.split(',').map(s => {
        ...
        // Een universele reset (`*{margin:0;padding:0}`) heeft in de bron
        // specificiteit 0. Gescoopt tot `.inhoud-body *` werd hij 0,1,0 en won
        // hij van elke component met één klasse die later niet meer komt:
        // .bgx-staptegel, .bgx-vertrek, .bgx-mensen, .bgx-tijdlijn en de rest
        // verloren hun binnenruimte en de tekst plakte tegen de rand (mobiel,
        // 65 van 87 pagina's). Onze eigen componenten (klasse begint met bgx-)
        // vallen daarom buiten de reset; voor al het andere blijft hij gelijk.
        if (/^\*/.test(t)) return `${scope} :where(*:not([class^="bgx-"]))${t.slice(1)}`;
        return `${scope} ${t}`;
      }).filter(Boolean).join(',');
...
// Tussenblokken (rekenmodule, vergelijker, clusterblok) komen vóór een h2.
// Staat die h2 in een kaart of een donker CTA-blok, dan belandde het blok
// daarbinnen: op mobiel stond de vergelijker in het slotblok van
// /investeerders-ma en in een kaart op /expertises, en het clusterblok in het
// CTA-blok van de blogs. Alleen koppen die in dezelfde container staan als de
// meeste andere koppen tellen daarom mee: dat is de lopende tekst.
export function kopPlekkenInDeHoofdstroom(tekst) {
  const bak = /<(\/?)(div|section|article|aside|header|footer|nav|main|form|ul|ol|table|details|figure|blockquote)\b[^>]*>|<h2[\s>]/gi;
  const stapel = [];
  const perContainer = new Map();
  for (const m of String(tekst).matchAll(bak)) {
    if (/^<h2/i.test(m[0])) {
      const houder = stapel.length ? stapel[stapel.length - 1] : -1;
      if (!perContainer.has(houder)) perContainer.set(houder, []);
      perContainer.get(houder).push(m.index);
    } else if (m[1]) stapel.pop();
    else stapel.push(m.index);
  }
  let beste = [];
  for (const plekken of perContainer.values()) if (plekken.length > beste.length) beste = plekken;
  return beste;
}
...
${intro ? `<p class="intro">${intro}</p>` : ''}
  ...
  function verdeel(tekst, blokken) {
    if ([...tekst.matchAll(/<h2[^>]*>/g)].length < blokken.length + 1) return tekst + blokken.join('');
    const koppenIn = kopPlekkenInDeHoofdstroom(tekst);
    // Past niet alles tussen de koppen van de lopende tekst, dan komt de rest
    // onderaan: liever daar dan in een kaart of slotblok.
    const aantal = Math.min(blokken.length, Math.max(0, koppenIn.length - 1));
    if (!aantal) return tekst + blokken.join('');
    const stap = Math.max(1, Math.floor(koppenIn.length / (aantal + 1)));
    let uit = '', vorig = 0;
    blokken.slice(0, aantal).forEach((blok, n) => {
      const punt = koppenIn[Math.min((n + 1) * stap, koppenIn.length - 1)];
      uit += tekst.slice(vorig, punt) + blok;
      vorig = punt;
    });
    return uit + tekst.slice(vorig) + blokken.slice(aantal).join('');
  }
  ...
  // De schil draagt de zoekwoordmeta van de homepage mee. Elke pagina krijgt
