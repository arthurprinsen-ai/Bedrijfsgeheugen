...
// Keep the accepted historical V18 page structure exactly from the pinned payload.
// Only the media source is made local/iPhone-safe so preview rendering does not
// depend on the retired external Pexels stream.
const heroMatch = html.match(/<video[^>]*id="heroBackgroundVideo"[^>]*>[\s\S]*?<\/video>/);
if (!heroMatch) throw new Error('Historical V18 hero video element missing');
let hero = heroMatch[0];
hero = hero.replace(/poster="[^"]*"/i, `poster="${HERO_POSTER_URL}"`);
hero = hero.replace(/<source\s+src="[^"]+"\s+type="video\/mp4"\s*\/?>/i, `<source src="${HERO_VIDEO_URL}" type="video/mp4">`);
if (!hero.includes(HERO_VIDEO_URL)) throw new Error('V18 preview hero media swap failed');
html = html.replace(heroMatch[0], hero);

// Test must be unmistakably distinct from production and must never be indexed.
html = html.replace('</head>', '  <meta name="robots" content="noindex,nofollow">\n  <meta name="bg-environment" content="V18-TEST">\n</head>');
html = html.replace('</body>', '<script>window.__BG_TEST_VERSION__="V18-HISTORICAL";</script>\n</body>');

await writeFile('prototype-v18-stable.html', html, 'utf8');
await writeFile('index.html', html, 'utf8');
console.log(`Historical V18 test preview built from pinned payload: ${EXPECTED_HTML_SHA256}`);
