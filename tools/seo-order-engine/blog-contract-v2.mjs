...
function normalizeContractForV1(input){return String(input).replace(/<meta\b[^>]*name=(?:"bg-order-contract"|'bg-order-contract')[^>]*content=(?:"v2"|'v2')[^>]*>/i,'<meta name="bg-order-contract" content="v1">');}
...
function markBody(input,commercial){const html=String(input);if(!/<body\b/i.test(html))return html;const intent=commercial?.primary_intent||'unassigned';const keyword=commercial?.primary_keyword||'unassigned';const owner=commercial?.route||'';return html.replace(/<body\b([^>]*)>/i,(_tag,attrs)=>{const clean=attrs.replace(/\sdata-bg-page-role=(?:"[^"]*"|'[^']*')/gi,'').replace(/\sdata-bg-funnel-stage=(?:"[^"]*"|'[^']*')/gi,'').replace(/\sdata-bg-intent=(?:"[^"]*"|'[^']*')/gi,'').replace(/\sdata-bg-keyword-cluster=(?:"[^"]*"|'[^']*')/gi,'').replace(/\sdata-bg-intent-role=(?:"[^"]*"|'[^']*')/gi,'').replace(/\sdata-bg-intent-owner=(?:"[^"]*"|'[^']*')/gi,'');return `<body${clean} data-bg-page-role="article" data-bg-funnel-stage="discover" data-bg-intent="${esc(intent)}" data-bg-keyword-cluster="${esc(keyword)}" data-bg-intent-role="supporting" data-bg-intent-owner="${esc(owner)}">`;});}

export function enrichBlog(input,path,registry){
  let html=enrichBlogV1(normalizeContractForV1(input),path,registry);
  ...
  html=ensureMeta(html,'bg-intent',commercial?.primary_intent||'unassigned');
  html=ensureMeta(html,'bg-keyword-cluster',commercial?.primary_keyword||'unassigned');
  html=ensureMeta(html,'bg-intent-owner',commercial?.route||'');
  return html;
}
...
export function inspectBlog(input,path,registry){
  ...
  const commercial=dominantCommercialEntry(html,registry);
  if(!commercial?.route)errors.push(`${path}: dominante commerciële intent-owner ontbreekt`);
  ...
  if(commercial?.route&&!new RegExp(`<meta\\b[^>]*name=(?:"bg-intent-owner"|'bg-intent-owner')[^>]*content=(?:"${commercial.route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}"|'${commercial.route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}')[^>]*>`,'i').test(headOf(html)))errors.push(`${path}: intent-owner meta ontbreekt`);
  if(!/data-bg-page-role=["']article["']/i.test(html))errors.push(`${path}: article page-role ontbreekt`);
  if(!/data-bg-intent-role=["']supporting["']/i.test(html))errors.push(`${path}: supporting intent-role ontbreekt`);
  if(commercial?.route&&!new RegExp(`data-bg-intent-owner=["']${commercial.route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}["']`,'i').test(html))errors.push(`${path}: supporting intent-owner ontbreekt`);
