...
(function(){
  ...
  function sessionId(){var key='bg:rum:session-id';try{var existing=sessionStorage.getItem(key);if(existing)return existing;var id='portal-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,10);sessionStorage.setItem(key,id);return id;}catch(e){return 'portal-'+Date.now().toString(36);}}
  async function start(){
    try{
      ...
      var token=await user.jwt();if(!token)return;
      var authHeaders={authorization:'Bearer '+token};
      var runtime=await import('/platform/client/portal-runtime.mjs');
      var revision=await runtime.resolveProductionRevision();if(!revision)return;
