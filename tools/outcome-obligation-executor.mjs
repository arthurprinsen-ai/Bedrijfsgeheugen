...
function normalizeTrigger(trigger) {
  if (!trigger || !SUPPORTED_TRIGGERS.has(trigger.type)) throw new TypeError(`unsupported trigger type: ${trigger?.type ?? 'missing'}`);
  const fingerprint = trigger.type === 'scheduled-sweep' ? String(trigger.fingerprint || 'scheduled') : requireText(trigger.fingerprint, 'trigger.fingerprint');
...
export function computeExecutionIdentity({ obligation, now, trigger, coalesceKey = null, timeZone = 'Europe/Amsterdam' }) {
  ...
  const executionWindow = businessDate(now, timeZone);
  ...
  return freeze({
    ...
    idempotencyKey,
    traceId:`obligation-trace|${stableDigest(idempotencyKey)}`,
  });
}
...
function exactProductionEvidence(evidence = []) {
  return acceptedEvidence(evidence).filter(item => item.exactProduction === true);
}
...
export function evaluateOutcomeObligation({ obligation, now, trigger, agent, due = true, priorWork = null, priorRecovery = null, evidence = [], hardBoundary = null, evidenceDeadline = null, productionProofRequired = false, coalesceKey = null, timeZone = 'Europe/Amsterdam' }) {
  ...
  const exactProduction = exactProductionEvidence(evidence);
  const base = {
  ...
  if (productionProofRequired && accepted.length > 0) return freeze({ ...base, status:'AWAITING_OUTCOME' });
  ...
  if (priorWork && expired) {
    return freeze({ ...base, status:'MISSED_OBLIGATION', recovery:{ type:'RecoveryWork', ownerAgent, obligationId:identity.obligationId, idempotencyKey:`recovery|${identity.idempotencyKey}`, policy:obligation.recoveryPolicy ?? null } });
  }
  if (priorWork) return freeze({ ...base, status:'AWAITING_OUTCOME' });
  return freeze({ ...base, status:'PENDING', dispatch:{ type:'AgentWork', ownerAgent, obligationId:identity.obligationId, idempotencyKey:identity.idempotencyKey, requestedOutcome:obligation.expected ?? null } });
}
