...
const CONTENT_TYPES=new Set(['blog','linkedin','social','page','unknown']);
...
const EVENT_FIELDS=['event_id','event_type','canonical','intent','intent_owner','attribution_root_key','source','medium','campaign','occurred_at','page_role','funnel_stage','content_id','content_type','channel','value','fingerprint'];
const OUTCOME_FIELDS=['outcome_id','stage','attribution_root_key','canonical','intent_owner','occurred_at','content_id','content_type','channel','revenue_eur','source','fingerprint'];
...
function contentType(value, contentId=''){const v=clean(value,40).toLowerCase();if(v&&CONTENT_TYPES.has(v))return v;if(clean(contentId).startsWith('blog:'))return 'blog';return 'unknown';}
...
export function normalizeGrowthEventForDataHub(input={}){
  ...
  for(const key of EVENT_FIELDS){if(['event_id','event_type','canonical','occurred_at','content_type'].includes(key))continue;const value=input[key];if(value===undefined||value===null||value==='')continue;if(key==='value'){const n=Number(value);if(!Number.isFinite(n))throw new Error('INVALID_EVENT_VALUE');out.value=n;}else out[key]=clean(value,key==='fingerprint'?300:500);}
  if(input.content_id||input.content_type)out.content_type=contentType(input.content_type,input.content_id);
...
export function normalizeGrowthOutcome(input={}){
  ...
  for(const key of OUTCOME_FIELDS){if(['outcome_id','stage','attribution_root_key','canonical','occurred_at','revenue_eur','content_type'].includes(key))continue;const value=input[key];if(value!==undefined&&value!==null&&value!=='')out[key]=clean(value,key==='fingerprint'?300:500);}
  if(input.content_id||input.content_type)out.content_type=contentType(input.content_type,input.content_id);
