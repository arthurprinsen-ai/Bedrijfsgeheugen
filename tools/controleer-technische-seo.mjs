...
const CANONICAL_OVERRIDES = new Map([
  ['blog/afas-koppeling/index.html', `${ORIGIN}/afas-koppeling`],
]);
...
export function verwachtCanonical(pad) {
  if (CANONICAL_OVERRIDES.has(pad)) return CANONICAL_OVERRIDES.get(pad);
...
function zonderNietGerenderdeInhoud(html) {
  return String(html)
...
function headVan(html) {
  const head = String(html).match(/<head\b[^>]*>([\s\S]*?)<\/head>/i);
  return head ? zonderNietGerenderdeInhoud(head[1]) : '';
}

function bodyVan(html) {
  const body = String(html).match(/<body\b[^>]*>([\s\S]*?)<\/body>/i);
  return body ? zonderNietGerenderdeInhoud(body[1]) : '';
}

function isNoindex(html) {
  const head = headVan(html);
  const robots = matches(head, /<meta\b[^>]*name=(?:"robots"|'robots')[^>]*>/gi);
...
export function controleerSeoHtml(input, pad) {
  ...
  const titels = matches(head, /<title\b[^>]*>([\s\S]*?)<\/title>/gi);
  if (titels.length !== 1 || !titels[0]?.[1]?.replace(/<[^>]+>/g, '').trim()) fouten.push(`${pad}: exact één niet-lege title vereist`);

  const descriptions = matches(head, /<meta\b[^>]*name=(?:"description"|'description')[^>]*>/gi);
  if (descriptions.length !== 1 || !attribuut(descriptions[0]?.[0], 'content').trim()) fouten.push(`${pad}: exact één niet-lege meta description vereist`);

  const robots = matches(head, /<meta\b[^>]*name=(?:"robots"|'robots')[^>]*>/gi);
  if (robots.length !== 1) fouten.push(`${pad}: exact één robots meta vereist`);
  else if (/(?:^|[,\s])noindex(?:[,\s]|$)/i.test(attribuut(robots[0][0], 'content'))) fouten.push(`${pad}: publieke SEO-pagina mag niet noindex zijn`);

  const canonicals = matches(head, /<link\b[^>]*rel=(?:"canonical"|'canonical')[^>]*>/gi);
  ...
  const h1s = matches(body, /<h1\b[^>]*>[\s\S]*?<\/h1>/gi);
  if (h1s.length !== 1) fouten.push(`${pad}: exact één H1 vereist, gevonden ${h1s.length}`);

  const anchors = matches(body, /<a\b[^>]*href=(?:"([^"]*)"|'([^']*)')[^>]*>/gi);
  ...
  if (pad !== 'index.html') {
    const kruimels = matches(body, /<(?:nav|ol)\b[^>]*(?:aria-label=(?:"Kruimelpad"|'Kruimelpad')|class=(?:"[^"]*\bbgkruim\b[^"]*"|'[^']*\bbgkruim\b[^']*'))[^>]*>/gi);
...
export async function controleerTechnischeSeo() {
  ...
  for (const pad of await seoBestanden()) {
    ...
    fouten.push(...controleerSeoHtml(html, pad));
  ...
  console.log(`Technische SEO OK: ${paginas.length} publieke pagina's; canonical, title, description, H1, breadcrumbs en interne links gecontroleerd`);
