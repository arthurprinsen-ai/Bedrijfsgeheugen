...
export function releaseConcurrencyKey({ workflow, prNumber } = {}) {
  const name = String(workflow || '').trim();
  const pr = Number(prNumber);
  if (!name || !Number.isInteger(pr) || pr <= 0) throw new TypeError('workflow and positive prNumber are required');
  return `${name}-pr-${pr}`;
}
