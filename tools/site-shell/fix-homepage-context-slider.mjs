...
const RUNTIME_SRC = '/assets/compare-slider-runtime-canonical-v14.js';
const CHANGE_FLOW_SRC = '/assets/homepage-change-flow-runtime-v1.js';
...
[data-bg-compare-slider] .bg-compare-knob{position:absolute!important;left:var(--bg-compare-split,50%)!important;top:50%!important;width:44px!important;height:44px!important;border-radius:999px!important;background:#fff!important;border:3px solid #FFE86B!important;box-shadow:0 4px 18px rgba(0,0,0,.20)!important;z-index:22!important;pointer-events:none!important;transform:translate(-50%,-50%)!important}
[data-bg-compare-slider]:focus-visible{outline:3px solid #2742D6!important;outline-offset:3px!important}
...
[data-bg-change-flow] [data-bg-change-step][data-bg-change-status="active"],[data-bg-change-flow] [data-bg-change-step][data-bg-change-status="done"]{opacity:1!important;filter:none!important}
...
@media(max-width:720px){
  ...
  [data-bg-compare-slider] .compare-handle,[data-bg-compare-slider] .compare-knob{display:none!important}
  [data-bg-compare-slider] .bg-compare-knob{display:block!important;width:42px!important;height:42px!important}
...
@media(min-width:721px) and (max-width:1100px){[data-bg-change-flow]{--bg-change-rail-width:58px}}
@media(prefers-reduced-motion:reduce){[data-bg-change-flow] [data-bg-change-step],.bg-change-step-fill,.bg-change-flow-check,[data-bg-compare-slider] .compare-copy{transition:none!important}}
</style>`;

const RUNTIME_TAG = `<script ${MARKER} src="${RUNTIME_SRC}"></script>`;
const CHANGE_FLOW_TAG = `<script data-bg-change-flow-runtime src="${CHANGE_FLOW_SRC}"></script>`;
...
export function applyHomepageContextSliderReadability(html){
  ...
  if(!next.includes('--bg-compare-split:var(--split,50%)')||
     ...
     !next.includes('bg-compare-knob')||
     !next.includes(RUNTIME_SRC)||
     !next.includes(CHANGE_FLOW_SRC)||
     ...
     (next.match(/<script data-bg-context-slider-readable\s+src="\/assets\/compare-slider-runtime-canonical-v14\.js"><\/script>/g)||[]).length!==1||
     (next.match(/<script data-bg-change-flow-runtime\s+src="\/assets\/homepage-change-flow-runtime-v1\.js"><\/script>/g)||[]).length!==1){
    throw new Error('Compare-slider readability guard kon niet volledig worden toegepast');
  }
  return next;
}

export { SLIDER_SELECTOR, RUNTIME_SRC, CHANGE_FLOW_SRC };