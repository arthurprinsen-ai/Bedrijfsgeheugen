...
async function run() {
  ...
  try {
    ...
    await mobileLanguage.waitFor({ state:'visible', timeout:5_000 }).catch(()=>{ throw new Error('visible mobile language select is missing'); });
    await Promise.all([
    ...
    await dutchSelect.waitFor({ state:'visible', timeout:5_000 }).catch(()=>{ throw new Error('visible Dutch language select is missing'); });
    await Promise.all([
