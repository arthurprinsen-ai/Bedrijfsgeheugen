...
function injectMobileLanguage(html) {
  ...
  // Compact bgkop pages use a mobile drawer without the V18 aside.
  if (/id=(["'])bgkopMob\1/i.test(html) || /class=(["'])[^"']*\bbgkop-mob\b[^"']*\1/i.test(html)) {
    ...
    const close = /(<\/div>\s*){1,3}(?=\s*<\/nav>|\s*<\/header>|\s*<main\b)/i;
    if (close.test(html)) return html.replace(close, MOBILE_LANGUAGE + '$&');
  }
