...
if (touched('.github/workflows/bg184-stateful-blocker-dedupe-tests.yml','tests/brain-bg184-stateful-blocker-dedupe.test.mjs','config/bg184-stateful-blocker-dedupe.json'))
  run('bg184-stateful-dedupe','node',['--test','tests/brain-bg184-stateful-blocker-dedupe.test.mjs']);

if (touched('.github/workflows/brain-foundation-verify.yml','brain/','scripts/brain/','tests/brain/','docs/brain/'))
  run('brain-foundation','node',['scripts/brain/test-all.mjs']);

// Business OS product contracts are change-scoped to Business OS implementation.
// Workflow-only edits are the single-flight migration itself and must not fan out
// into unrelated product regressions; platform/portal-next changes still run both.
...
if (touched('.github/workflows/config-wacht.yml','netlify.toml','_redirects','sitemap.xml','tools/config-wacht.py'))
  run('config-watch','python3',['tools/config-wacht.py']);

if (touched('.github/workflows/fresh-device-autonomy-canary.yml','scripts/brain/device-certify.mjs','tests/brain-fresh-device-canary.test.mjs'))
  run('fresh-device-contract','node',['--test','tests/brain-fresh-device-canary.test.mjs']);

if (touched('.github/workflows/hero-media-production-verify.yml','assets/openart-hero-iphone-safe-v1.mp4','assets/openart-hero-production.json'))
...
if (touched('.github/workflows/homepage-hero-video-verify.yml','index.html','tests/homepage-hero-video.test.mjs'))
  run('homepage-hero-video','node',['tests/homepage-hero-video.test.mjs']);

if (touched('.github/workflows/main-write-integrity-regression.yml','tools/main-write-integrity.mjs','tests/main-write-integrity-squash-pr.test.mjs'))
  run('main-write-integrity','node',['--test','tests/main-write-integrity-squash-pr.test.mjs']);

if (touched('.github/workflows/portal-native-regression-tests.yml','portal/','tests/portal-native-legacy-batch-'))
  run('portal-native-regression','bash',['-lc','node --test tests/portal-native-legacy-batch-*.test.mjs']);

if (touched('.github/workflows/portal-v2-tests.yml','portal-v2/','portal-next/portal-content-map.js'))
  run('portal-v2-contracts','npm',['--prefix','portal-v2','test']);

if (touched('.github/workflows/prijzen-hero-seo-regression.yml','tools/site-shell/','tests/prijzen-hero-seo.test.mjs','tests/technical-seo-gate.test.mjs'))
  run('pricing-hero-seo','node',['--test','tests/prijzen-hero-seo.test.mjs','tests/technical-seo-gate.test.mjs']);

if (touched('.github/workflows/seo-growth-intelligence.yml','.github/workflows/seo-order-engine.yml','tools/seo-growth/','tools/seo-order-engine/','tests/seo-growth-','tests/seo-order-'))
  run('seo-order-growth','bash',['-lc','node --test tests/seo-order-*.test.mjs tests/seo-growth-*.test.mjs']);

if (touched('.github/workflows/universal-closed-loop-learning.yml','config/universal-closed-loop-learning.json','scripts/brain/validate-universal-closed-loop-learning.mjs','tests/brain-universal-closed-loop-learning.test.mjs')) {
...
if (touched('.github/workflows/universal-event-retention-contract.yml','tools/universal-event-','tools/universal-failure-','tools/event-retention-'))
...
if (touched('.github/workflows/verify-approved-central-blog.yml','tests/approved-blog-verifier-contract.test.mjs'))
  run('approved-blog-verifier','node',['--test','tests/approved-blog-verifier-contract.test.mjs']);

if (touched('.github/workflows/blog-technical-seo-gate.yml','blog/','robots.txt','sitemap.xml')) {
...
process.stdout.write(`${JSON.stringify({ ok: true, version: 'single-flight-preserved-contracts-v4', changedCount: changed.length, selected: [...selected] })}\n`);