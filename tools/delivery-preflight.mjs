...
export async function loadDeliveryPreflight({
  ...
  const [lessonsDoc, chatLessonsDoc, executionLessonsDoc, rulesDoc] = await Promise.all([
  ...
  if (executionLessonsDoc.version !== chatLessonsDoc.version || executionLessonsDoc.appendOnly !== true) {
    throw new Error('current execution lessons must remain an append-only BRAIN-CHAT-LEARNING-v1 shard');
  }
  ...
  const materialChatLessons = [...(chatLessonsDoc.lessons || []), ...(executionLessonsDoc.lessons || [])].map(lesson => ({
    ...
    preventionRule: lesson.preventionRule || lesson.id,
    status: 'PROVEN',
  }));
  const provenLessons = [...historicalLessons, ...materialChatLessons];
