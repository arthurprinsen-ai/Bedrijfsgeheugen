...
if (!hero.includes('/assets/inspirational-hero-v6.mp4')) fail('hero video must use cache-busted same-origin v6 MP4');
if (!hero.includes('poster="/assets/inspirational-hero-v6-poster.jpg"')) fail('hero v6 must have a same-origin poster');
if (html.includes('id="heroVideoFallback"')) fail('Framer-style v6 must not ship custom fallback controls');
if (html.includes('debugVideo')) fail('Framer-style v6 must not ship runtime debug UI');
if (html.includes('playbackRate=.65')) fail('Framer-style v6 must not alter playback rate');
if (!html.includes('IntersectionObserver')) fail('hero v6 must pause/resume based on viewport visibility');
if (!html.includes("rootMargin:'240px 0px 240px 0px'")) fail('hero v6 must preload near the viewport');
...
const VIDEO_PATH = 'assets/inspirational-hero-v6.mp4';
const POSTER_PATH = 'assets/inspirational-hero-v6-poster.jpg';
const videoStat = await stat(VIDEO_PATH);
const posterStat = await stat(POSTER_PATH);
if (videoStat.size < 250000) fail(`hero v6 unexpectedly small: ${videoStat.size} bytes`);
if (videoStat.size > 5000000) fail(`hero v6 exceeds 5 MB Framer loop budget: ${videoStat.size} bytes`);
if (posterStat.size < 10000) fail(`hero v6 poster unexpectedly small: ${posterStat.size} bytes`);
const videoBytes = await readFile(VIDEO_PATH);
if (videoBytes.subarray(4, 8).toString('ascii') !== 'ftyp') fail('hero v6 is not a valid MP4 container');
const videoHash = sha256(videoBytes);
if (!/^[a-f0-9]{64}$/.test(videoHash)) fail('hero v6 hash invalid');
...
if (audioLines.length !== 0) fail(`hero v6 must contain no input audio streams, found ${audioLines.length}`);
const videoLine = videoLines[0];
if (!/Video:\s*h264\b/.test(videoLine)) fail(`hero v6 is not H.264: ${videoLine.trim()}`);
if (!/\byuv420p\b/.test(videoLine)) fail(`hero v6 is not yuv420p: ${videoLine.trim()}`);
if (!/\b1280x720\b/.test(videoLine)) fail(`hero v6 is not 1280x720: ${videoLine.trim()}`);
if (!/Input #0, .*mp4/i.test(inputInfo)) fail('hero v6 input container was not recognized as MP4');
const durationMatch = inputInfo.match(/Duration:\s*(\d+):(\d+):(\d+(?:\.\d+)?)/);
if (!durationMatch) fail('hero v6 duration missing from ffmpeg inspection');
const duration = Number(durationMatch[1])*3600 + Number(durationMatch[2])*60 + Number(durationMatch[3]);
if (!(duration > 7 && duration < 9)) fail(`hero v6 duration ${duration}s outside expected source range`);

console.log(`V18 preview QA PASS — stable root, 14 views, ${targets.length} routes, Neno/Framer-style hero-v6 ${videoStat.size} bytes ${videoHash}, poster ${posterStat.size} bytes, h264/yuv420p/1280x720/no-audio/<5MB, ${hrefs.length} HTTPS anchors`);
