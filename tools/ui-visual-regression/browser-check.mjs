      ...
      new PerformanceObserver(list => {
        for (const entry of list.getEntries()) {
          if (!entry.hadRecentInput) {
            ...
            window.__bgUiVrCls.entries.push({ value: entry.value, startTime: entry.startTime });
          }
        }
      ...
      for (const viewport of registry.defaults.viewports) {
        ...
        for (const state of states) {
          ...
          try {
            ...
            if (sample.cls.value > registry.defaults.clsMax) violations.push({ ruleId: 'cls', actual: sample.cls.value, limit: registry.defaults.clsMax });
          } catch (error) {
