...
async function assertVisibleAndReadable(locator, { minOpacity = 0.45 } = {}) {
  const evidence = await locator.evaluate(el => {
    ...
    return { display: style.display, visibility: style.visibility, opacity: Number(style.opacity || 1), width: rect.width, height: rect.height };
  });
  if (evidence.display === 'none' || evidence.visibility === 'hidden' || evidence.opacity < minOpacity || evidence.width <= 0 || evidence.height <= 0) {
