...
function push(errors, contractId, path, expected) {
  errors.push({ contractId, code: 'build-wiring-loss', path, expected });
}
...
export function checkFinalInteractionWiring({ rootDir = process.cwd(), contracts = interactionContracts } = {}) {
  ...
  if (netlify && !/node tools\/prijzen-uit-de-homepage\.mjs/.test(netlify)) {
    push(errors, '<global>', 'netlify.toml', 'node tools/prijzen-uit-de-homepage.mjs');
  }
  ...
  for (const contract of contracts) {
    ...
    for (const marker of req.sourceMarkers) {
      if (!source.includes(marker)) push(errors, contract.id, req.builder, marker);
    }
  }
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/'))) {
  ...
  if (!result.ok) {
    ...
    console.log('Final interaction wiring OK');
  }
}
