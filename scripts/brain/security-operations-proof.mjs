...
export function assertIsolatedRestoreTarget({targetEnvironment='',targetProjectRef='',productionProjectRef=''}){
  const target=String(targetEnvironment).toLowerCase();
  const targetRef=String(targetProjectRef).toLowerCase();
  const prod=String(productionProjectRef).toLowerCase();
  const unsafe=target==='production'||target==='prod'||target.includes('production')||(prod&&target.includes(prod))||(prod&&targetRef===prod);
...
export function buildSecurityOperationsReport(input={}){
  const checks={database_hardening:validateDatabaseHardening(input.database_hardening),owner_mfa:validateOwnerMfa(input.owner_mfa),dr_restore:validateDrProof(input.dr_restore),credential_rotations:validateCredentialRotations(input.credential_rotations),iam_review:validateIamReview(input.iam_review),engineering_gates:validateEngineeringGates(input.engineering_gates)};
