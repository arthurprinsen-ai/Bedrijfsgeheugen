...
assert.ok(packet.sources.length <= 41, 'preflight must remain bounded to 41 sources');
...
assert.throws(
  () => compileChatLearningPreflight({ rootDir: process.cwd(), maxSources: 40, maxBytes: 256_000 }),
  /maxSources exceeded: 41 > 40/,
  'the previous 40-source ceiling must fail closed now that Engineering OS is a mandatory preflight authority'
);

assert.throws(
  () => compileChatLearningPreflight({ rootDir: process.cwd(), maxSources: 41, maxBytes: 100 }),
  /maxBytes/,
