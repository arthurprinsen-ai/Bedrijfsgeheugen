...
assert.match(canary,/workflow_dispatch:/,'cheap canary must remain explicitly agent-dispatchable');
assert.doesNotMatch(canary,/\bpull_request:/,'cheap canary must not consume a second automatic pull_request runner under single-flight');
assert.match(canary,/base_sha:/,'cheap canary must retain exact-base dispatch support');
assert.match(canary,/inputs\.base_sha \|\| 'main'/,'cheap canary must checkout the requested exact base or current main');
assert.match(canary,/createWriterCandidate/);
assert.match(canary,/validateWriterCandidate/);
assert.match(canary,/gh pr create/,'cheap canary must still create a verification candidate PR when explicitly dispatched');
...
console.log('PASS verification-only writer safety remains agent-dispatchable without extra PR runner and propagates cheap canary -> immutable shadow -> central gates -> Unified Brain -> no BG169');
