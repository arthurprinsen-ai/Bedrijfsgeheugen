...
export function detectFlakyTests(observations = []) {
  ...
  const flaky = [...groups.values()]
    ...
    .map(group => ({
      ...
      observations: group.outcomes.length,
      fail_rate: group.outcomes.filter(x => x === 'fail').length / group.outcomes.length,
      classification: 'same_revision_oscillation',
    }))
    .sort((a, b) => a.test_id.localeCompare(b.test_id) || a.source_revision.localeCompare(b.source_revision));
  return {
    flaky,
    flake_rate: groups.size ? flaky.length / groups.size : null,
    can_override_required_failure: false,
    policy: 'diagnostic_only_never_rerun_until_green',
  };
}
...
export function computeEngineeringScorecard(input = {}) {
  ...
  const evidenceCompleteness = all.filter(metric => metric.status === 'measured').length / all.length;
  return {
    fingerprint: 'powerhouse-engineering-scorecard-v1',
    metrics,
    evidence_completeness: evidenceCompleteness,
    truth_rule: 'unknown_is_not_zero',
  };
}
...
export function computeBlastRadius(graph, seeds = []) {
  ...
  while (queue.length) {
    ...
    for (const [dependent, dependsOn] of graph?.edges ?? []) {
      if (dependsOn === dependency && !impacted.has(dependent)) {
...
export function evaluateRecoveryProof({ now = new Date().toISOString(), max_age_days = 90, required_domains = [], evidence = [] } = {}) {
  ...
  return {
    recovery_proven: blocking.length === 0 && required_domains.length > 0,
    max_age_days,
    domains,
    blocking_domains: blocking.sort(),
    policy: 'fresh_successful_bounded_rehearsal_required',
  };
}
...
export function createGoldenPathScaffold(kind, name) {
  ...
  return {
    kind,
    name: slug,
    files: scaffoldDefinitions[kind](slug),
    hooks: ['contract', 'test', 'observability', 'documentation', 'rollback', 'readback'],
...
export function evaluateEngineeringMetaLearning({ gates = [], escaped_defects = [], flake_rate = null } = {}) {
  ...
  for (const defect of escaped_defects) {
    recommendations.push({ gate: defect.should_have_been_caught_by ?? null, action: 'add_or_strengthen_regression_from_escaped_defect', evidence: { fingerprint: defect.fingerprint ?? 'unknown' }, promotion: 'protected_delivery_required' });
  }
  ...
  return {
    ...
    recommendations,
    promotion_rule: 'all_changes_use_existing_engineering_golden_path',
  };
}
...
async function main() {
  ...
  if (mode === '--scaffold') {
    const scaffold = createGoldenPathScaffold(arg1, arg2);
    if (arg3 === '--write') scaffold.written = await writeScaffold(scaffold);
  ...
  const handlers = {
    ...
    '--graph': value => {
      const graph = buildDependencyGraph(value.graph ?? value);
      return { graph, blast_radius: computeBlastRadius(graph, value.seeds ?? []) };
    },
  ...
  if (!handler) {
    console.error('Usage: --self-test | --scorecard [file] | --flakes [file] | --graph [file] | --recovery [file] | --learn [file] | --scaffold <kind> <name> [--write]');
    process.exitCode = 2;
    return;
  }
