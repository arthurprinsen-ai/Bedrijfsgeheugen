...
export function buildStablePolicyPrefix() {
  return Object.freeze({ version: FAST_EXECUTION_VERSION, defaultEnabled: true, existingStateFirst: true, reuseFirst: true, canonicalIntegration: true, closedLoop: true, hotStateFirst: true, deltaFirst: true, lazyTools: true, parallelReadOnlyRetrieval: true, deterministicBeforeModel: true, boundedToolOutput: true, evidenceReuseWithFreshness: true, terminalState: 'LIVE & BEWEZEN' });
}
...
export class EvidenceCache {
  ...
  metadata(key) { const item = this.items.get(key); return item ? { observedAt: item.observedAt, expiresAt: item.expiresAt, source: item.source, candidateId: item.candidateId } : null; }
}
...
export class LatencyTrace {
  ...
  snapshot() { return { ...this.durations, 'total-ms': Math.max(0, this.now() - this.createdAt) }; }
}
