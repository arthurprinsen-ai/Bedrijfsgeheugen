...
const MANDATORY_SUPPLEMENTAL_SOURCES = [
  ...
  'brain/learning/chat-execution-addendum-preflight-2026-08-31.json'
];
...
function measurePacket(packet) {
  let measured = { ...packet, totalBytes: 0 };
  for (let attempt = 0; attempt < 4; attempt += 1) {
    const bytes = Buffer.byteLength(JSON.stringify(measured), 'utf8');
    if (measured.totalBytes === bytes) return measured;
    measured = { ...measured, totalBytes: bytes };
  }
  return measured;
}
...
export function compileChatLearningPreflight({
  ...
  if (packet.totalBytes > maxBytes) throw new Error(`maxBytes exceeded: ${packet.totalBytes} > ${maxBytes}`);
  return packet;
}
