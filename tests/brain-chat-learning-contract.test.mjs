...
const requiredLessonIds = [
  'STALE_MAIN_REBUILD_DONT_FORCE','BIND_EVIDENCE_TO_EXACT_SHA_AND_RUN','SEMANTIC_TESTS_IGNORE_SQL_COMMENTS',
  'CROSS_MIGRATION_GRANTS_ARE_ONE_CONTRACT','LINTER_INFO_REQUIRES_WORKLOAD_EVIDENCE',
  'IMMUTABLE_TABLES_REVOKE_TRUNCATE_AND_DDL_PRIVS','SERVICE_ROLE_LEAST_PRIVILEGE_EXPLICIT_GRANTS',
  'CONNECTOR_LIMITATION_IS_HARD_BOUNDARY_NOT_SUCCESS','NETLIFY_READY_IS_NOT_EXACT_SHA_PROOF',
  'API_DEPLOY_SOURCE_DOES_NOT_IMPLY_GIT_AUTODEPLOY','CONCURRENCY_422_INSPECT_BEFORE_OVERWRITE',
  'NO_NOOP_PRODUCTION_CHANGE_FOR_CANARY_PROOF','NEW_TOOL_PREFIX_MUST_BE_BRAIN_CLASSIFIED',
  'WRITERS_ARE_CANDIDATE_ONLY_NO_DIRECT_MAIN','NOTION_IS_PROJECTION_GITHUB_IS_EXECUTABLE_TRUTH',
  'EVENT_DRIVEN_BEFORE_POLLING_FOR_COST','RLS_OPTIMIZATION_MUST_PRESERVE_RUNTIME_SEMANTICS',
...
test('chat-derived failures are machine-readable reusable BRAIN lessons', async () => {
  ...
  assert.equal(config.policy.matchBeforeDebugging, true);
  assert.equal(config.policy.reuseKnownFixBeforeExperimenting, true);
  assert.equal(config.policy.blockKnownFailedApproachWithoutNewEvidence, true);
...
test('mandatory delivery preflight reuses every chat-derived lesson before expensive work', async () => {
  const preflightSource = await readFile('tools/delivery-preflight.mjs', 'utf8');
