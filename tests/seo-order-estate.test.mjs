...
const registry = {
  version: 2,
  pages: [
    { route: `${ORIGIN}/prijzen`, role: 'money', primary_intent: 'kosten digitalisering mkb', primary_keyword: 'kosten digitalisering mkb', secondary_keywords: [], funnel_stage: 'decide', search_intent: 'commercial', target_page_type: 'money', business_goal: 'lead', priority: 5, primary_cta: { action: 'frisse-blik', url: `${ORIGIN}/frisse-blik` }, supporting_routes: [`${ORIGIN}/blog/kosten/`], schema_type: 'Service' }
  ]
};
...
function measuredMoney(){
  return injectGrowthMeasurement(enrichRegisteredPage(moneyHtml,registry.pages[0]),{canonical:`${ORIGIN}/prijzen`,page_role:'money',funnel_stage:'decide',intent:'kosten digitalisering mkb',keyword_cluster:'kosten digitalisering mkb',intent_owner:`${ORIGIN}/prijzen`,search_intent:'commercial',business_goal:'lead',target_page_type:'money',priority:5});
}
function measuredSupport(){
  ...
  html=injectGrowthMeasurement(html,{canonical:`${ORIGIN}/blog/kosten/`,page_role:'support',funnel_stage:'discover',intent:'kosten digitalisering uitleg',keyword_cluster:'kosten digitalisering uitleg',intent_owner:`${ORIGIN}/prijzen`,search_intent:'informational',business_goal:'assisted-conversion',target_page_type:'guide',priority:5});
...
function measuredAlias(){
  ...
  html=injectGrowthMeasurement(html,{canonical:`${ORIGIN}/prijzen`,page_role:'article',funnel_stage:'discover',intent:'oude prijzen uitleg',keyword_cluster:'oude prijzen uitleg',intent_owner:`${ORIGIN}/prijzen`,search_intent:'informational',business_goal:'assisted-conversion',target_page_type:'guide',priority:5});
