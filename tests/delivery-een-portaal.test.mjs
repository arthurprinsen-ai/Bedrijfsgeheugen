...
test('alle demo-aliases canonicaliseren naar dezelfde Portal V2 demo-ingang', () => {
  const tekst = regels().join('\n');
...
test('echte klantslugs canonicaliseren naar Portal V2 en niet naar legacy html', () => {
  const tekst = regels().join('\n');
  assert.match(tekst, /^\/klantportaal\s+klant=:klant\s+\/portaal\/:klant\s+301!$/m,
    'echte klanten moeten via de canonieke Portal V2-ingang binnenkomen');
  assert.doesNotMatch(tekst, /^\/klantportaal\s+klant=:klant\s+\/klantportaal\.html\s+200!$/m,
    'legacy klantportaal.html mag niet opnieuw canonieke entry worden');
});
