...
async function createIsolatedLearningFixture(dir, { lessons = [], rules = [] } = {}) {
  ...
  const websiteRestorationLessonsPath = join(dir, 'website-restoration-lessons.json');
  await Promise.all([
    ...
    writeFile(websiteRestorationLessonsPath, JSON.stringify({ version: 'BRAIN-CHAT-LEARNING-v1', appendOnly: true, lessons: [] })),
  ]);
  return { lessonsPath, rulesPath, chatLessonsPath, continuityPath, executionLessonsPath, websiteRestorationLessonsPath };
}
