...
test('latest material chat learnings are persisted in the canonical Brain preflight', () => {
  ...
  assert.ok(paths.has('brain/learning/chat-execution-addendum-preflight-2026-08-31.json'));
  ...
  assert.ok(packet.preventions.includes('REPLAY_BRAIN_WRITEBACK_ONCE_AFTER_CAPACITY_RECOVERY'));
  assert.ok(packet.preventions.includes('SINGLE_CANONICAL_WRITER_HANDOFF'));
  assert.ok(packet.blockers.includes('BLOCKED_HARD_BOUNDARY'));
});
