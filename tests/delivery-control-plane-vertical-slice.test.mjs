...
test('terminal closure fails closed when canonical production readback is unavailable', async()=>{
  const workflow=await readFile('.github/workflows/obligation-terminal-closure.yml','utf8');
  ...
  assert.doesNotMatch(workflow,/PRODUCTION_DESCENDANT_READBACK_PROVEN|mode=descendant_live|curl .*release\.json/);
});

test('terminal evidence has one production-readback authority', async()=>{
  const workflow=await readFile('.github/workflows/obligation-terminal-closure.yml','utf8');
  ...
  assert.doesNotMatch(workflow,/mode=descendant_live|production-descendant:/);
});
