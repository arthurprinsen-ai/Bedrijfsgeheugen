...
const migrationPath = 'supabase/migrations/20260830183706_brain_outcome_obligation_store_strict_privileges.sql';
...
test('strict privilege migration never grants truncate trigger references update or delete', async () => {
  const sql = (await readFile(migrationPath, 'utf8')).toLowerCase();
  for (const forbidden of ['grant truncate', 'grant trigger', 'grant references', 'grant update', 'grant delete', 'grant all']) assert.equal(sql.includes(forbidden), false, `${forbidden} must not be present`);
});
