...
test('all global capabilities are native, mobile-safe and fail closed without auth',async({page})=>{
 ...
 for(const capability of expected){
  const control=page.locator(`[data-capability="${capability}"]`).first();
  await expect(control,capability).toBeVisible();
 ...
 await expect(page.locator('[data-capability="identity-login-logout"]')).toHaveText(/Inloggen|Uitloggen/);
 await page.locator('[data-capability="feedback"]').click();
 await expect(page.locator('.v2globalstatus')).toContainText('Log in om deze actie veilig uit te voeren.');
