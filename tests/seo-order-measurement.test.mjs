...
test('generated growth measurement inline script is valid JavaScript',()=>{
  const base='<html><body><main><a href="https://www.bedrijfsgeheugen.nl/zelfscan" data-bg-conversion="zelfscan">Start</a></main></body></html>';
  const out=injectGrowthMeasurement(base,{canonical:'https://www.bedrijfsgeheugen.nl/test'});
  const match=out.match(/<script id="bg-growth-measurement">([\s\S]*?)<\/script>/);
  assert.ok(match,'growth measurement script must be injected');
  assert.doesNotThrow(()=>new vm.Script(match[1],{filename:'bg-growth-measurement.js'}));
});
