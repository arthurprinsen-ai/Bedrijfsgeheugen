...
import {
  allowedForWriter,
  validateWriterPaths,
} from '../scripts/ci/repo-writer-policy.mjs';
...
test('authoritative policy covers all seven direct-main writers', () => {
  ...
  for (const writer of writers) {
    assert.ok(allowedForWriter(writer).length > 0, `${writer} must have a bounded path policy`);
  }
...
test('writer path policies accept intended files and reject cross-domain drift', () => {
  ...
  assert.equal(validateWriterPaths('approved-central-blog', [
    'blog/voorbeeld/index.html',
    'blog/index.html',
    'blog/rss.xml',
    'sitemap.xml',
  ]).ok, true);
  ...
  assert.throws(
    () => validateWriterPaths('regelgeving-bijwerken', ['data/regelgeving.json', '.github/workflows/x.yml']),
    /UNAPPROVED_WRITER_PATH/,
  );
  assert.throws(
    () => validateWriterPaths('menu-balk-fix', ['index.html', 'package.json']),
    /UNAPPROVED_WRITER_PATH/,
  );
});
