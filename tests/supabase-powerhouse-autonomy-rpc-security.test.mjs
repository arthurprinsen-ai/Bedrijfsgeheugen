...
test('autonomous Powerhouse RPCs are fail-closed and deterministic',()=>{
  ...
  assert.match(sql,/to_regprocedure\('public\.powerhouse_autonomous_growth_revenue_cycle\(date\)'\)/i);
  assert.match(replaySql,/revoke execute on function public\.powerhouse_autonomous_growth_revenue_cycle\(date\)[\s\S]*from public, anon, authenticated/i);
  assert.match(replaySql,/grant execute on function public\.powerhouse_autonomous_growth_revenue_cycle\(date\)[\s\S]*to service_role/i);
