...
test('scorecard keeps missing evidence unknown and measures Powerhouse lead time', () => {
  const result = computeEngineeringScorecard({
    changes: [
      { id: 'c1', idea_at: '2026-09-16T08:00:00Z', committed_at: '2026-09-16T08:10:00Z', deployed_at: '2026-09-16T08:30:00Z', live_proven_at: '2026-09-16T08:40:00Z', failed: false },
    ],
  });
...
test('dependency graph computes reverse blast radius deterministically', () => {
  const graph = buildDependencyGraph({
    nodes: ['code:a', 'test:a', 'workflow:req', 'runtime:web'],
    edges: [
      ['test:a', 'code:a'],
      ['workflow:req', 'test:a'],
      ['runtime:web', 'code:a'],
...
test('meta-learning recommends evidence-backed improvements but never mutates gates', () => {
  const result = evaluateEngineeringMetaLearning({
    gates: [
      { id: 'visual', runs: 100, caught_defects: 8, false_failures: 0, median_duration_ms: 120000 },
      { id: 'legacy', runs: 100, caught_defects: 0, false_failures: 14, median_duration_ms: 600000 },
    ],
