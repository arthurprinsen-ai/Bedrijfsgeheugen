...
test('Verified Value prefers explicit verified Value records while preserving legacy Outcome compatibility', () => {
  const explicit=normalizeBrainRecord({tenantId:'T1',type:'Value',id:'VAL1',subjectId:'s:1',owner:'agent',executed:true,verified:true,result:'saved',evidenceIds:['EV1'],payload:{realised:true,realisedValue:125,valueUnit:'EUR'}});
  const rejected=normalizeBrainRecord({tenantId:'T1',type:'Value',id:'VAL2',subjectId:'s:2',owner:'agent',executed:true,verified:false,result:'claim',evidenceIds:['EV2'],payload:{realised:true,realisedValue:999,valueUnit:'EUR'}});
  const legacy=normalizeBrainRecord({tenantId:'T1',type:'Outcome',id:'O1',subjectId:'s:3',owner:'agent',executed:true,verified:true,result:'legacy',evidenceIds:['EV3'],payload:{realisedValue:25,valueUnit:'EUR'}});
  const projection=projectVerifiedValue([explicit,rejected,legacy]);
  assert.deepEqual(projection.verifiedValues.map(x=>x.id),['VAL1']);
  assert.deepEqual(projection.legacyVerifiedOutcomes.map(x=>x.id),['O1']);
  assert.equal(projection.totals.EUR,150);
});

test('Living Memory exposes explicit Memory records separately with freshness and provenance', () => {
  const memory=normalizeBrainRecord({tenantId:'T1',type:'Memory',id:'M1',subjectId:'s:1',owner:'BG167',observedAt:'2026-08-31T07:00:00Z',evidenceIds:['EV1'],payload:{learningId:'L1'},source:'brain',sourceId:'M1'});
  const evidence=normalizeBrainRecord({tenantId:'T1',type:'Evidence',id:'E1',subjectId:'s:1',owner:'sensor',observedAt:'2026-08-20T07:00:00Z',evidenceIds:[],payload:{},source:'sensor',sourceId:'E1'});
  const projection=projectLivingMemory([memory,evidence],{now:'2026-08-31T07:30:00Z',defaultMaxAgeMs:86400000});
  assert.deepEqual(projection.memories.map(x=>x.id),['M1']);
  assert.equal(projection.memories[0].freshness.status,'fresh');
