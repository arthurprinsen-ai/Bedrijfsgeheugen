...
test('mobile language injector supports v18 and compact bgkop mobile drawers', async () => {
  ...
  assert.match(source,/html\\.replace\\(cta, MOBILE_LANGUAGE \\+ '\\  assert.match(source,/html\.replace\(cta, MOBILE_LANGUAGE \+ '\\  assert.match(source,/html\.replace\(cta, match => MOBILE_LANGUAGE \+ match\)/);'\)/);
});'\\)/);
});
