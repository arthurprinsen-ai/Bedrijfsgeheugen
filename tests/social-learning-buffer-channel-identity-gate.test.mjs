...
test('business exception is permanently disabled on personal LinkedIn',()=>{const text='Ik stond thuis vanochtend met mijn laptop en dacht aan mijn consultancy ervaring.';assert.equal(run(text).authorized,false);const attempted=run(text,{businessException:{explicitUserRequest:true,contentId:'personal-1',singleUse:true}});assert.equal(attempted.authorized,false);assert.ok(attempted.reasons.includes('BUSINESS_CONTENT_ON_PERSONAL'))});
