...
const lineage={contentId:'personal-1',calendarDate:'2026-09-21',predictionId:'pred-1',sourceDecisionId:'decision-1'};
const truth={class:'author_observation',verified:true,evidenceRefs:['chat:verified-personal-fact']};
const run=(text,extra={})=>authorizeSocialPublication({channelKind:'linkedin_personal',channelId:CHANNELS.linkedin_personal.channelId,text,lineage,personalTruth:truth,companyPageInterchangeable:false,personalLifeOnlyVerified:true,...extra});

test('V4 and exact channel identities are canonical',()=>{
 assert.equal(CHANNEL_IDENTITY_CONTRACT_ID,'arthur-personal-linkedin-identity-v4');
...
for(const [name,text,reason] of [
 ...
 ['forced lesson','Ik stond thuis met de printer. Dit geldt ook voor organisaties.','FORCED_BUSINESS_MORAL']
]) test(name+' is blocked',()=>{const r=run(text);assert.equal(r.authorized,false);assert.ok(r.reasons.includes(reason))});
...
test('wrong Buffer channel fails closed',()=>{const r=authorizeSocialPublication({channelKind:'linkedin_personal',channelId:CHANNELS.linkedin_company.channelId,text:'Ik heb thuis ruzie met mijn printer.',lineage,personalTruth:truth,companyPageInterchangeable:false,personalLifeOnlyVerified:true});assert.equal(r.authorized,false);assert.ok(r.reasons.includes('CHANNEL_IDENTITY_MISMATCH'))});
...
test('business content can never be excepted onto personal LinkedIn',()=>{const text='Ik stond thuis vanochtend met mijn laptop en dacht aan mijn consultancy ervaring.';assert.equal(run(text).authorized,false);const stillBlocked=run(text,{businessException:{explicitUserRequest:true,contentId:'personal-1',singleUse:true}});assert.equal(stillBlocked.authorized,false);assert.ok(stillBlocked.reasons.includes('BUSINESS_CONTENT_ON_PERSONAL'))});
test('missing prediction/content lineage blocks publication',()=>{const r=authorizeSocialPublication({channelKind:'linkedin_personal',channelId:CHANNELS.linkedin_personal.channelId,text:'Ik heb thuis ruzie met mijn printer.',lineage:{contentId:'x'},personalTruth:truth,companyPageInterchangeable:false,personalLifeOnlyVerified:true});assert.equal(r.authorized,false);assert.ok(r.reasons.includes('LINEAGE_INCOMPLETE'))});
...
const validInstagramVisual={
 ...
 evidenceRefs:['vision:final-frame'],assetUrl:'https://cdn.example/final.mp4',placeholderDetected:false,
 identityClass:'mira_daily_life',formatVerified:true,width:1080,height:1920,frameEvidence:frames
};
const instagram=(extra={})=>authorizeSocialPublication({
 ...
 assetUrl:'https://cdn.example/final.mp4',assetMimeType:'video/mp4',...extra
});

test('Instagram cannot publish with Mira gate alone; final visual evidence is mandatory',()=>{
 const r=instagram(); assert.equal(r.authorized,false); assert.ok(r.reasons.includes('INSTAGRAM_VISUAL_EVIDENCE_REQUIRED'));
});
test('Instagram blocks a detected placeholder or broken render',()=>{
 ...
 assert.equal(r.authorized,false); assert.ok(r.reasons.includes('INSTAGRAM_PLACEHOLDER_BLOCKED'));
});
test('Instagram blocks visuals that do not match Mira daily-life identity',()=>{
 ...
 assert.equal(r.authorized,false); assert.ok(r.reasons.includes('INSTAGRAM_MIRA_VISUAL_REQUIRED'));
});
test('Instagram blocks when inspected asset is not the exact asset sent to provider',()=>{
 const r=instagram({instagramVisual:{...validInstagramVisual,assetUrl:'https://cdn.example/other.mp4'}});
...
test('Instagram authorizes only the exact verified Mira Reel',()=>{
 const r=instagram({instagramVisual:validInstagramVisual}); assert.equal(r.authorized,true);
});
