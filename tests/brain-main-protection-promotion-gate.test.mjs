...
test('production writer handoff fails closed when live main protection is not verified', async () => {
  ...
  const bg169Handoff = yaml.indexOf('Hand exact green lane identities to Brain production authority');
  assert.ok(protectionGate >= 0 && bg169Handoff >= 0 && protectionGate < bg169Handoff,
    'live main protection must be verified before BG169 handoff');
});
