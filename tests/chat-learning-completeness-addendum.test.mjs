...
test('chat learning completeness addendum retains current Make recovery contracts', async () => {
  const raw = await readFile('brain/learning/chat-completeness-addendum-2026-08-30.json', 'utf8');
  const addendum = JSON.parse(raw);

  assert.equal(addendum.version, 'CHAT-LEARNING-COMPLETENESS-ADDENDUM-v1.2');
  ...
  for (const guard of ['canonical_owner','material_state_change_before_dispatch','global_single_flight','cache_first_worker_read','state_hash_before_write','bounded_credit_slope','readback_before_retry_after_429_or_502','learning_write_fail_open_from_projection_refresh']) {
  ...
  const artifactDrift = addendum.failurePatterns.find(x => x.id === 'CANONICAL_ARTIFACT_REFERENCE_DRIFT');
  assert.match(artifactDrift.requiredAction, /exact persisted path/i);
  ...
  for (const required of ['new durable fingerprint stored','root cause recorded','prevention and regression contract recorded','open recovery obligations explicitly preserved','machine-readable CI/preflight coverage exists','canonical artifact paths verified by exact readback','optional observability and learning paths cannot own primary business result delivery','multi-step structural migrations are atomic or every intermediate state is independently safe']) {
