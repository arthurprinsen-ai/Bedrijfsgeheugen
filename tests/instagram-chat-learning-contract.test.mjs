...
const MEMORY_WORKFLOW_PATH = new URL('../.github/workflows/shared-agent-memory-tests.yml', import.meta.url);
...
test('shared agent memory CI executes the Instagram/Make learning contract test', async () => {
  const workflow = await readFile(MEMORY_WORKFLOW_PATH, 'utf8');

  assert.match(
    workflow,
    /tests\/instagram-chat-learning-contract\.test\.mjs/,
    'Shared Agent Memory Tests must execute this regression contract on pull requests and main',
  );
});
