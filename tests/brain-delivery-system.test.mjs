...
test('canonical architecture specs are governed delivery-control-plane work, never ignored documentation', async () => {
  const policy = JSON.parse(await readFile('config/brain-delivery-system.json', 'utf8'));
  ...
  assert.ok(deriveConflictContracts([canonicalPath], policy).includes('delivery-control-plane'));
});
