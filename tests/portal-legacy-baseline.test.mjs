...
test('every concrete legacy workspace remains represented',()=>{const ids=new Set(LEGACY_WORKSPACES.map(x=>x.id));for(const id of workspaces)assert.equal(ids.has(id),true,`missing legacy workspace ${id}`)});
...
test('concrete legacy workspace hashes resolve into integrated bridge routes',()=>assert.deepEqual(resolvePortalRoute('profiel'),{route:'company/legacy/profiel',legacyId:'profiel'}));
test('native capability route wins where a workspace name is also native',()=>assert.deepEqual(resolvePortalRoute('roadmap'),{route:'execution/roadmap',legacyId:'roadmap'}));
