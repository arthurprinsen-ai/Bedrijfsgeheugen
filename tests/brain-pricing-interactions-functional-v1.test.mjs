...
test('mobile taps use a CSP-safe external delegated runtime',async()=>{
  const [html,runtime]=await Promise.all([
    pricing(),
    readFile(new URL('../assets/js/pricing-interactions-live-v3.js',import.meta.url),'utf8')
  ]);
  assert.match(html,/pricing-interactions-live-v3\.js\?v=20260923-1900/);
...
test('pricing rescue v3 survives DOM replacement and initializes immediately', async () => {
  const source = await readFile(new URL('../assets/js/pricing-interactions-live-v3.js', import.meta.url), 'utf8');
  assert.match(source, /__BG_PRICING_RESCUE_V3__/);
