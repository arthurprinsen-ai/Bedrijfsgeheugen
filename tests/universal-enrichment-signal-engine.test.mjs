  ...
  const providers = [
    {
      ...
      async enrich() {
        ...
        return {
          industry: evidence('provider-a', 'Manufacturing', 0.78),
          employee_count: evidence('provider-a', 220, 0.72),
        };
      },
    },
    {
      ...
      priority: 20,
      async enrich() {
        calls.push('provider-b');
        return {
          industry: evidence('provider-b', 'Manufacturing', 0.82),
          employee_count: evidence('provider-b', 228, 0.73),
        };
      },
    },
    {
      ...
      async enrich() {
        ...
        return { industry: evidence('provider-c', 'Industrial', 0.9) };
      },
    },
  ];

  const result = await enrichKnowledgeEntity({
    entity: { id: 'company:acme', kind: 'company', facts: {} },
    providers,
...
test('waterfall records contradictions rather than silently overwriting canonical knowledge', async () => {
  const result = await enrichKnowledgeEntity({
    entity: {
      id: 'person:1',
      kind: 'person',
      facts: { role: evidence('crm', 'CIO', 0.92) },
    },
    providers: [
      { id: 'external', priority: 10, async enrich() { return { role: evidence('external', 'CTO', 0.9) }; } },
    ],
    requiredFields: ['role'],
...
test('data health exposes coverage, freshness, conflicts and a deterministic next refresh', () => {
  const health = calculateDataHealth({
    ...
    requiredFields: ['industry', 'employee_count', 'erp'],
    now: () => at,
    staleAfterDays: 30,
  });
