...
test('daily blog watchdog recovers from historical reruns against current main', () => {
  assert.match(workflow, /uses: actions\/checkout@v5[\s\S]*?ref: main[\s\S]*?fetch-depth: 0/);
  assert.match(workflow, /if \(days\[today\]\) \{ console\.log\(today\); process\.exit\(0\); \}/);
  assert.match(workflow, /Object\.keys\(days\)\.filter\(d=>d<=today\)\.sort\(\)/);
