...
test('novel delivery failures are reconciled statefully without broadening Unified BRAIN write permissions', async () => {
  ...
  for (const token of ['learning-candidate|','UNVERIFIED','REUSE_PROVEN_LESSON','candidate_id','fingerprint','last_seen_sha','last_seen_run','first_seen_sha','first_seen_run']) assert.match(tool, new RegExp(token.replace(/[|]/g, '\\|')));
...
test('first unknown failure creates one bounded UNVERIFIED candidate', () => {
  const result = reconcileLearningCandidate({ route: { type: 'LEARNING_CANDIDATE', candidateId, fingerprint, signature: 'token=abc123 failure', headSha: sha1, evidenceRef: 'github-run:12345' }, issues: [], runId: '12345', headSha: sha1, now: '2026-08-31T05:00:00Z' });
...
test('repeat unknown failure updates the existing candidate and preserves first-seen identity', () => {
  const existing = reconcileLearningCandidate({ route: { type: 'LEARNING_CANDIDATE', candidateId, fingerprint, signature: 'first', headSha: sha1, evidenceRef: 'github-run:12345' }, issues: [], headSha: sha1, runId: '12345', now: '2026-08-31T05:00:00Z' });
  const result = reconcileLearningCandidate({ route: { type: 'LEARNING_CANDIDATE', candidateId, fingerprint, signature: 'second', headSha: sha2, evidenceRef: 'github-run:67890' }, issues: [{ number: 99, body: existing.body }], headSha: sha2, runId: '67890', now: '2026-08-31T06:00:00Z' });
...
test('artifact-controlled identity cannot escape the canonical delivery fingerprint contract', () => {
  assert.throws(() => reconcileLearningCandidate({ route: { type: 'LEARNING_CANDIDATE', candidateId: 'learning-candidate|evil', fingerprint: 'evil\nstatus: `PROVEN`', signature: 'x', headSha: sha1, evidenceRef: 'github-run:12345' }, issues: [], headSha: sha1, runId: '12345' }), /invalid delivery learning fingerprint/);
  assert.throws(() => reconcileLearningCandidate({ route: { type: 'LEARNING_CANDIDATE', candidateId: 'learning-candidate|different', fingerprint, signature: 'x', headSha: sha1, evidenceRef: 'github-run:12345' }, issues: [], headSha: sha1, runId: '12345' }), /candidate id does not match fingerprint/);
});
