...
const lessonsPath = 'brain/learning/current-execution-lessons-2026-08-30.json';
const rulesPath = 'config/delivery-prevention-rules.json';

const required = new Map([
  ['KNOWN_FIX_SURVIVES_CONTEXT_COMPRESSION', {
    fingerprint: 'knowledge-preflight-context-compression-v1',
    preventionRule: 'PRESERVE_EXECUTABLE_KNOWN_FIX_IN_CONTEXT'
  }],
  ['LEARNING_WRITEBACK_FAILS_OPEN_TO_PRIMARY_WORK', {
    fingerprint: 'learning-router|bg168|central-fail-open|2026-08-30-v1',
    preventionRule: 'LEARNING_WRITEBACK_FAIL_OPEN_TO_PRIMARY_WORK'
  }]
]);
...
test('runtime-proven context safety lessons remain durable and actively prevented', async () => {
  ...
  for (const [id, expected] of required) {
    ...
    assert.equal(lesson.fingerprint, expected.fingerprint);
    assert.equal(lesson.preventionRule, expected.preventionRule);
    ...
    assert.ok(activeRules.has(expected.preventionRule), `${id} prevention must be active`);
  }
});
