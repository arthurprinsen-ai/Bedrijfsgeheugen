...
import { evaluateWriterLease } from '../tools/delivery/delivery-hygiene.mjs';
...
test('PR machine metadata is exact-once while admission tolerates unrelated main movement',()=>{
  ...
  const lease=evaluateWriterLease({body:staleEpochBody,candidateHeadSha:B,currentMainSha:A,obligationId:'obligation-1'});
  assert.equal(lease.ok,true);
  const terminalLease=evaluateWriterLease({body:staleEpochBody,candidateHeadSha:B,currentMainSha:A,obligationId:'obligation-1',enforceCurrentMainEpoch:true});
  assert.equal(terminalLease.ok,false);
  assert.ok(terminalLease.reasons.includes('TERMINAL_LEASE_MAIN_EPOCH_DRIFT'));
});
