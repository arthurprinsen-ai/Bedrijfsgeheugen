...
test('footer contract is machine-readable and SEO governed', () => {
  ...
  assert.equal(c.rules.noDeadFooterLinks, true);
  assert.equal(c.rules.noKeywordCannibalization, true);
  assert.ok(Array.isArray(c.exceptions));
  assert.ok(c.exceptions.every(x => x.file && x.reason));
  for (const route of ['/bedrijfsgeheugen','/bedrijfsprocessen-automatiseren','/afas-koppeling','/ai-governance','/data-soevereiniteit','/benchmark']) {
    assert.ok(c.strategicDestinations.includes(route), `missing strategic footer destination ${route}`);
  }
});
