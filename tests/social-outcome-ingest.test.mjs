...
function req(body, method='POST') { return new Request('https://example.test/api/social-outcome-ingest',{method,headers:{'content-type':'application/json'},body:method==='POST'?JSON.stringify(body):undefined}); }
...
test('ingest persists audit event before analytical snapshot', async()=>{
  const order=[];
  const handler=createSocialOutcomeHandler({persistEvent:async()=>{order.push('event');return {status:'RECEIVED'};},store:{putPost:async()=>order.push('post'),appendSnapshot:async()=>order.push('snapshot')}});
...
test('invalid payload is rejected without writes', async()=>{
  let writes=0;
  const handler=createSocialOutcomeHandler({persistEvent:async()=>writes++,store:{putPost:async()=>writes++,appendSnapshot:async()=>writes++}});
...
test('GET is not allowed', async()=>{
  const handler=createSocialOutcomeHandler({persistEvent:async()=>{},store:{}});
