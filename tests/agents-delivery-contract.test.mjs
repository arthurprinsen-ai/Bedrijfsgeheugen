...
test('nonterminal CI completion contract forbids stopping before verified production outcome', async () => {
  ...
  for (const key of ['ciPendingIsTerminal', 'ciFailureIsTerminal', 'openPrIsTerminal', 'mergeWaitIsTerminal', 'deployInProgressIsTerminal', 'missingProductionReadbackIsTerminal']) {
    assert.equal(contract.policy[key], false, `${key} must remain false`);
  }
  for (const state of ['queued', 'pending', 'in_progress', 'failed_check', 'open_pr', 'merge_wait', 'deploy_in_progress', 'production_drift', 'missing_production_readback']) {
