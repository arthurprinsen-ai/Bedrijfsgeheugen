...
assert.deepEqual(reachedFromGeometry(760, [100, 400, 700, 1000]), [true, true, true, false]);
assert.deepEqual(reachedFromGeometry(1010, [100, 400, 700, 1000]), [true, true, true, true]);
assert.deepEqual(reachedFromGeometry(150, [100, 400, 700, 1000]), [true, false, false, false]);
...
assert.match(once, /lijn\.end\+2>=starts\[i\]/);
assert.equal(injectHomepageProcessProgress(once), once, 'build patch must be idempotent');
