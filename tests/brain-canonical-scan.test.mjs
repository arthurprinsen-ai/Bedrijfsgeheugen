...
test('public scan proxy blocks privileged history and claim actions',async()=>{const source=await readFile(new URL('../netlify/functions/powerhouse-scan-ingest.mjs',import.meta.url),'utf8');assert.match(source,/PRIVILEGED_ACTION_FORBIDDEN/);assert.match(source,/body\?\.action==='history'/);assert.match(source,/body\?\.action==='claim'/);});
