...
import { loadEngineeringContract, validateEngineeringOS } from '../tools/powerhouse-engineering-os.mjs';
...
test('Development OS has no stale BRAIN-DELIVERY-v1 authority', async () => {
  ...
  assert.match(content, /node tools\/powerhouse-engineering-os\.mjs --check/);
});
...
test('Required test executes this regression contract', async () => {
  ...
  assert.match(workflow, /tests\/powerhouse-engineering-os-contract\.test\.mjs/);
});

test('CLI emits READY only after the complete contract validates', () => {
  const output = execFileSync(process.execPath, ['tools/powerhouse-engineering-os.mjs', '--check'], { encoding: 'utf8' });
