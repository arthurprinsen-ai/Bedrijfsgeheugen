...
test('iedere huidige sitemap-pagina heeft expliciete SEO/CRO-classificatie', async()=>{
...
test('toekomstige onbekende niet-blog pagina faalt gesloten', async()=>{
  const registry=await loadRegistry();
  assert.equal(classifyCanonical(`${ORIGIN}/nieuwe-commerciele-pagina`,registry),null);
});
...
test('money contract bevat direct antwoord FAQ methode en update-signaal',()=>{
  const entry={route:`${ORIGIN}/ai-implementeren`,role:'money',primary_intent:'ai implementeren mkb',primary_keyword:'ai implementeren mkb',secondary_keywords:[],funnel_stage:'decide',primary_cta:{action:'frisse-blik',url:`${ORIGIN}/frisse-blik`},supporting_routes:[`${ORIGIN}/ai-adoptie`],schema_type:'Service'};
