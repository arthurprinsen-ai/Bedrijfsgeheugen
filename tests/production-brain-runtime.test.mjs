...
import {createProviderRegistry} from '../platform/policy/provider-registry.mjs';
...
test('production factory returns projected runtime with async mutation boundary',async()=>{
 const layers=new Map();const store={getLayer:async(t,l)=>layers.get(`${t}:${l}`)||null,putCanonical:async(t,p)=>{layers.set(`${t}:canonical-brain`,p);return{stored:true,record:p}}};
 const providerRegistry=createProviderRegistry([{id:'p:m',provider:'p',model:'m',allowedDataClasses:['PUBLIC'],region:'EU',contractStatus:'APPROVED'}]);
 ...
 const p=runtime.ingest({id:'S1',tenantId:'t1',provenance:{sourceType:'API',sourceRef:'src'},idempotencyKey:'i1'},{actorId:'a'});assert.equal(typeof p.then,'function');await p;
