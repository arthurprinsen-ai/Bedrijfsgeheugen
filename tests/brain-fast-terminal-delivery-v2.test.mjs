...
test('terminal closure fails closed on a known failed canonical readback and uses descendant proof only for cancelled or missing readback', async()=>{
  ...
  assert.match(workflow,/PRODUCTION_READBACK_FAILED:run=\$source_run_id conclusion=\$source_conclusion/);
