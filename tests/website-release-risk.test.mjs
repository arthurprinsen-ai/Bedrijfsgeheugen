...
test('megamenu implementation changes require the focused menu browser gate', () => {
  assert.equal(profile(['tools/bouw-v18-production-core.mjs']), 'full');
  assert.equal(profile(['tools/site-shell/v18-megamenu-heading-contract.mjs']), 'full');
  assert.equal(profile(['tools/site-shell/v18-megamenu-browser-check.mjs']), 'full');
});
...
test('full surface risk dominates targeted and control-plane changes', () => {
  assert.equal(profile([
    '.github/workflows/required-test.yml',
    'index.html',
    'assets/stijl.js'
