...
test('normale klantportaal-slugs gaan naar de tenant-aware portal',()=>{
  assert.match(redirects,/\/klantportaal\s+klant=:klant\s+\/portal\/\s+200!/);
...
test('ijsselmonde gebruikt dezelfde canonical portalroute',()=>{
  assert.match(redirects,/\/klantportaal\s+klant=ijsselmonde\s+\/portal\/\s+200!/);
});
