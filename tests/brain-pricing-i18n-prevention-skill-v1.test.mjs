...
test('public language switches fail closed to in-place translation on unprefixed routes', () => {
  assert.match(i18n,/const routed = pathLocale\(\);[\s\S]*if \(routed\) \{[\s\S]*location\.assign\(localizedHref\(normalized\)\)/);
  assert.match(i18n,/Unprefixed public routes must switch in place/);
  assert.match(i18n,/locale = normalized;[\s\S]*await apply\(document\.body\)/);
