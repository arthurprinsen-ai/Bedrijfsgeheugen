...
const migrationPath = 'supabase/migrations/20260830133911_benchmark_views_security_invoker.sql';

async function migrationText() {
  try { return await readFile(migrationPath, 'utf8'); } catch { return ''; }
}
...
test('benchmark views execute with caller privileges via security_invoker', async () => {
  ...
  for (const view of ['benchmark_branche', 'benchmark_niveaus', 'benchmark_offertes']) {
    assert.match(sql, new RegExp(`alter\\s+view\\s+public\\.${view}\\s+set\\s*\\(\\s*security_invoker\\s*=\\s*true\\s*\\)`, 'i'), `${view} must use security_invoker=true`);
  }
});
