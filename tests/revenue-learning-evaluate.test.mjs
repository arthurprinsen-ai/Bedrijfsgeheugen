...
test('revenue evidence supersedes stronger click engagement',async()=>{
  const learnings=[];
  ...
  const store={listDueEvidence:async()=>[target],listCohort:async()=>cohort,upsertLearning:async l=>learnings.push(l),recordObligation:async()=>{},reconcileApplications:async()=>{}};
...
test('missing comparable evidence creates obligation instead of fake zero',async()=>{
  const obligations=[];
  const store={listDueEvidence:async()=>[{evidenceId:'e1',contentId:'li:1',channel:'linkedin_company',windowHours:72,componentFingerprint:'hook:tension',exposures:1000}],listCohort:async()=>[],upsertLearning:async()=>{},recordObligation:async o=>obligations.push(o),reconcileApplications:async()=>{}};
