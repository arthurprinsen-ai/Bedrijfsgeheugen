...
test('revenue evidence supersedes stronger click engagement and checkpoints the evaluation',async()=>{
  ...
  const store={...projectionDefaults,listDueEvidence:async()=>[target],listCohort:async()=>cohort,upsertLearning:async l=>learnings.push(l),recordObligation:async()=>{},reconcileApplications:async()=>{},markEvidenceEvaluated:async(id,at)=>marked.push({id,at})};
...
test('commercially positive evidence can promote after thresholds are met',async()=>{
  ...
  const store={...projectionDefaults,listDueEvidence:async()=>[target],listCohort:async()=>cohort,upsertLearning:async l=>learnings.push(l),recordObligation:async()=>{},reconcileApplications:async()=>{},markEvidenceEvaluated:async()=>{}};
  await createRevenueEvaluator({store,config:{promotion:{minSampleSize:5,minPublicationDates:2,minConfidence:.75},metricPriority:['revenue','orders','proposals','qualified_leads','meetings','leads','clicks','substantive_interactions']},now:()=>new Date('2026-09-09T00:00:00Z')})();
  assert.equal(learnings[0].status,'PROVEN');
  assert.equal(learnings[0].effectMetric,'revenue');
});

test('evaluator refreshes bounded proven projection for the next content decision even with no due evidence',async()=>{
  let projection;
  ...
  const store={listDueEvidence:async()=>[],listCurrentLearnings:async()=>learnings,putProjection:async p=>{projection=p;}};
  const result=await createRevenueEvaluator({store,now:()=>new Date('2026-09-09T00:00:00Z')})();
  assert.equal(result.total,0);
  assert.equal(projection.learnings.length,8);
