...
test('authenticated ingest writes immutable Brain authority first, feeds Powerhouse and persists canonical projection', async () => {
  ...
  assert.equal(response.status,200);assert.equal(authorityWrites.length,4);
  const [raw,source,current,impact]=authorityWrites;
  assert.equal(raw.record.tenantId,'tenant-real');assert.equal(raw.record.type,'SourceObservation');assert.equal(raw.record.kind,'raw_source');
  assert.equal(source.record.tenantId,'tenant-real');assert.equal(source.record.type,'BusinessInput');assert.equal(source.record.kind,'SourceTruth');assert.equal(source.record.subjectId,'PORTAL_INPUT-StrategyModel-business-model-primary');
  assert.equal(current.record.type,'CurrentState');assert.deepEqual(current.record.predecessorIds,[source.record.id]);
  assert.equal(impact.record.type,'ImpactAssessment');assert.equal(impact.record.kind,'organism_impact');assert.deepEqual(impact.record.predecessorIds,[current.record.id]);assert.deepEqual(impact.record.evidenceIds,[source.record.id,current.record.id]);
  assert.equal(stored.tenantId,'tenant-real');assert.equal(stored.record.origin,'canonical-brain');assert.equal(stored.record.data.businessInputs[0].truthClass,'SourceTruth');assert.equal(stored.record.data.businessInputs[0].answers.customer,'SME');assert.equal(stored.record.data.businessInputs[0].metadata.brainRecordId,payload.brainRecordId);assert.equal(stored.record.data.businessInputs[0].metadata.currentStateRecordId,payload.currentStateRecordId);assert.equal(payload.authorityStored,true);assert.equal(payload.powerhouseFeedStored,true);assert.equal(payload.stored,true);
});
