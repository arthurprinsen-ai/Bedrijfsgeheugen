...
test('failed commit PR merge and pipeline outcomes become reusable deterministic lessons', () => {
  const lesson = createFailureLesson({
    stage: 'PR', reason: 'Required check Brain Delivery failed for changed portal route',
    rootCause: 'portal regression test was not run before opening the PR',
    fix: 'run portal targeted preflight before opening PR', preventionRule: 'TARGETED_PORTAL_PREFLIGHT',
    component: 'portal', headSha: 'abcdef1234567890',
  });
